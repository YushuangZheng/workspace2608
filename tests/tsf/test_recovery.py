...
def test_offline_link_reentry_uses_all_persistent_origin_states(recovery_case) -> None:
    model, _, _ = recovery_case
    event_id, anchor = next(iter(model.link_anchors.items()))
    persistent = tuple(
        sorted(
            key.state_id
            for key, origin in model.link_origins.items()
            if origin == event_id
        )
    )
    assert persistent
    assert set(anchor.linked_entry_states).issubset(persistent)

    runtime = EpisodeLinkAnchorRegistry(model).resolve(
        event_id.frame_id,
        persistent[-1],
        event_id.mode,
    )
    assert runtime.legal_reentry_states == persistent
...
def test_non_task_belief_update_freezes_progress_and_updates_relation(
    ...
    model, demonstrations, _ = recovery_case
    state = StateId(1, 2)
...
def test_frozen_update_censors_frame_follow_as_relation_evidence(
    recovery_case,
) -> None:
    model, demonstrations, _ = recovery_case
    previous_state = StateId(1, 1)
    current_state = StateId(1, 2)
    previous = observation_for(model, demonstrations, previous_state, 5)
    current = replace(
        observation_for(model, demonstrations, current_state, 6),
        previous_ee_pose=previous.ee_pose,
        previous_command_pose=observation_for(
            model, demonstrations, current_state, 6
        ).ee_pose,
    )
    updater = BeliefUpdater(model)
    initial = np.asarray([0.95, 0.05])
    updater.reset(
        initial_progress={current_state: 1.0},
        initial_relations={"object": initial},
        initial_relation_decisions={
            "object": RelationDecision.EXTERNAL,
        },
        previous_observation=previous,
    )
    ...
    result = updater.update_frozen(
        current,
        mode_by_skill={1: 0},
        uninformative_relation_frames=frozenset({"object"}),
    )

    assert result.runtime_features.actual_motion_magnitude > 0.0
    assert result.runtime_features.relation_information_weight["object"] == 0.0
    estimate = result.relation_estimates["object"]
    np.testing.assert_allclose(estimate.posterior, estimate.predicted)
    assert estimate.informative is False
    assert result.progress.posterior == {current_state: 1.0}

    with pytest.raises(KeyError, match="未知参考系"):
        updater.reset(
            initial_progress={current_state: 1.0},
            initial_relations={"object": initial},
            previous_observation=previous,
        )
        updater.update_frozen(
            current,
            mode_by_skill={1: 0},
            uninformative_relation_frames=frozenset({"not_a_frame"}),
        )
...
def test_pending_verification_uses_candidate_relation_context_without_moving_beta(
    ...
    model, demonstrations, pending = recovery_case
...
def test_verify_link_probes_opposite_approach_returns_and_blocks_repeat(
    ...
    static_features = replace(
        features_for(*recovery_case[:2], pending.candidate_state),
...
def test_verify_link_confirmation_commits_original_filter_posterior(
    ...
    model, _demonstrations, _pending = recovery_case
    updater = BeliefUpdater(model)
...
def test_pending_failed_link_uses_provisional_template_then_activates_on_success(
    ...
    recovery = __import__(
        "essay2608.policy.tsf", fromlist=["RelationRecoveryController"]
    ).RelationRecoveryController(
        registry,
        RecoveryConfig(
...
def test_runtime_does_not_invent_unlink_without_learned_release_metadata(
    ...
    manager = TSFRecoveryManager(model)
...
def test_unlink_uses_informative_detachment_evidence_at_static_target(
    recovery_case,
) -> None:
    model, demonstrations, _ = recovery_case
    registry = EpisodeLinkAnchorRegistry(model)
    goal = RelationGoalPlanner(
        registry, UnlinkMetadataRepository(model)
    ).plan(
        (
            RelationRecoveryIntent(
                "single",
    ...
    recovery = RelationRecoveryController(
        ...
        RecoveryConfig(
            pose_position_tolerance=1.0e-6,
            maximum_waypoint_cycles=3,
            maximum_relation_verify_cycles=3,
            minimum_relation_confirmation_cycles=3,
        ),
    )
    recovery.start((goal,))
    frame = demonstrations[0].frames["object"][10]
    current = demonstrations[0].ee_pose[10]

    # Reach the detachment target while the persistent filter still reports
    # LINKED.  The first static EXTERNAL decision is deliberately
    # uninformative and must not be accepted as proof of release.
    result = None
    for _ in range(10):
        result = recovery.update(
            current_pose=current,
        ...
        if result.goal_phase is not None and result.goal_phase.value == "verify":
            break
    assert result is not None and result.phase == RecoveryPhase.EXECUTING
    uninformative = recovery.update(
        current_pose=current,
        current_gripper=np.asarray([1.0]),
        frame_poses={"object": frame},
        relation_estimates={
    ...
    assert uninformative.phase == RecoveryPhase.EXECUTING

    # One informative response to the already executed detachment motion is
    # causal evidence.  Holding the same target on later cycles cannot create
    # more excitation, so the persistent EXTERNAL decision completes UNLINK.
    informative = recovery.update(
        current_pose=current,
        current_gripper=np.asarray([1.0]),
        frame_poses={"object": frame},
        relation_estimates={"object": relation(RelationDecision.EXTERNAL)},
    )
    assert informative.phase == RecoveryPhase.REENTRY
    assert informative.completed_goals == (goal,)
...
def test_recovery_failure_has_hard_attempt_limit(recovery_case) -> None:
    model, demonstrations, _ = recovery_case
    registry = EpisodeLinkAnchorRegistry(model)
    goal = RelationGoalPlanner(registry, UnlinkMetadataRepository(model)).plan(
        (
...
def test_link_recovery_waits_for_and_locks_a_stable_object_frame(recovery_case) -> None:
    model, demonstrations, _ = recovery_case
    registry = EpisodeLinkAnchorRegistry(model)
    goal = RelationGoalPlanner(registry, UnlinkMetadataRepository(model)).plan(
        (
...
def test_reentry_uses_full_state_and_requires_cross_skill_guard(recovery_case) -> None:
    model, demonstrations, _ = recovery_case
    ...
    updater = BeliefUpdater(model)
    execution = TSFExecutionController(model)
...
def test_reentry_reuses_recovery_covariance_without_changing_normal_scores(
    ...
    zero_runtime_variance = StateEvaluatorConfig(robot_runtime_variance=0.0)
    strict = ReentrySelector(
        model,
        config,
        evaluator_config=zero_runtime_variance,
    ).select(
        (state,),
        displaced,
        current_reference=state,
...
def test_manager_modes_pending_activation_and_persistent_recovery_trigger(
    ...
    # A stable external verification is a reliable failed-LINK result.  It
    # must enter the existing learned LINK recovery path instead of returning
    # to TASK and suppressing the same verification forever.
    unresolved_external_belief = replace(
        belief,
        relation_estimates={
            "object": relation(RelationDecision.EXTERNAL, information=0.0)
        },
    )
    second_manager = TSFRecoveryManager(
        model,
        TSFRecoveryConfig(
            verification=RelationVerificationConfig(
                probe_speed=0.1,
    ...
    assert step.verification is not None
    assert step.verification.phase == VerificationPhase.COMPLETE
    assert step.verification.decision == RelationDecision.EXTERNAL
    assert step.mode == ExecutionMode.RECOVERY
    assert second_manager.frozen_reference == pending.candidate_state
    goal = second_manager.recovery.active_goal
    assert goal is not None
    assert goal.kind == RelationGoalKind.LINK
    assert goal.link_anchor is not None
    assert goal.link_anchor.origin_event_id == pending.event_id
    assert "object" not in second_manager.anchor_registry.active_pending
...
def test_manager_accepts_unresolved_formal_link_verification(recovery_case) -> None:
    model, demonstrations, _ = recovery_case
    ...
    step = manager.update_verification(belief, current_pose=pose(0.02))
    external_belief = replace(
        belief,
        relation_estimates={anchor.frame_id: relation(RelationDecision.EXTERNAL)},
        runtime_features=verification_response_features(
            belief.runtime_features,
            ee_translation=-0.005,
            frame_translation=0.0,
    ...
    assert step.verification.decision == RelationDecision.EXTERNAL
    assert step.mode == ExecutionMode.RECOVERY
    goal = manager.recovery.active_goal
    assert goal is not None
    assert goal.kind == RelationGoalKind.LINK
    assert goal.link_anchor is not None
    assert goal.link_anchor.origin_event_id == event_id
    assert goal.link_anchor.source == "offline_link"


def test_manager_verifies_exact_committed_pending_occurrence_with_stale_external(
    recovery_case,
) -> None:
    """Pre-close posterior memory must not hide a causally committed event."""
    ...
    belief = belief_for(
        ...
        demonstrations,
        pending.candidate_state,
        relation(RelationDecision.EXTERNAL, information=0.8),
        motion=0.0,
    )
    belief = replace(
        belief,
        runtime_features=replace(
            ...
            relation_information_weight={pending.frame_id: 0.8},
        ),
    )
    ordinary = RelationVerificationRequest(
        pending.arm_id,
        pending.frame_id,
        "linked",
        pending.event_id,
    ...
    committed = replace(ordinary, committed_occurrence=True)

    assert not manager.can_begin_verification(
        ordinary,
        belief,
        task_state=pending.candidate_state,
        grasp_event=pending.event_id.token,
    )
    assert manager.can_begin_verification(
        committed,
        belief,
...
def test_manager_reentry_atomically_resets_progress_and_reference(
    ...
    manager = TSFRecoveryManager(
        model,
        TSFRecoveryConfig(reentry=ReentryConfig(minimum_explanation_score=0.0)),
    )
...
def test_same_state_reentry_ablation_resumes_frozen_reference_without_search(
    recovery_case,
) -> None:
    model, demonstrations, _ = recovery_case
    source = StateId(2, 2)
    ...
    evaluation = manager.select_same_state_reentry(
        belief,
        mode_by_skill={source.skill_index: 0},
    )

    assert evaluation.decision is not None
    assert evaluation.decision.state_id == source
    assert evaluation.decision.crossed_boundary is None
    assert evaluation.rejection_reasons == {}
    assert tuple(evaluation.scores) == (source,)


def test_recovery_config_file_matches_code_defaults() -> None:
    loaded = TSFRecoveryConfig.from_json("configs/tsf_recovery.json")
    assert loaded.to_dict() == TSFRecoveryConfig().to_dict()
    with pytest.raises(ValueError, match="未知分区"):
        TSFRecoveryConfig.from_mapping({"unexpected": {}})
