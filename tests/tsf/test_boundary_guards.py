...
from essay2608.policy.tsf.control.entry_guard import _factor_observation
from essay2608.policy.tsf.inference.state_evaluator import (
...
def test_local_goal_support_uses_the_joint_poe_target_distribution() -> None:
    ...
    observed = relative_pose(frame, joint_mean)
    compatibility, current_log_support, peak_log_support = (
        joint_peak_normalized_pose_support(
            [
                (
                    "first",
    ...
    assert current_log_support == pytest.approx(peak_log_support, abs=1.0e-12)


def test_runtime_covariance_broadens_joint_support_without_moving_poe_mean() -> None:
    frame = pose(0.0)
    first_mean = pose(0.0)
    second_mean = pose(0.04)
    first_covariance = np.eye(6) * 1.0e-4
    second_covariance = np.eye(6) * 4.0e-4
    marginals = [
        transform_marginal("first", frame, first_mean, first_covariance),
        transform_marginal("second", frame, second_mean, second_covariance),
    ]
    joint_mean, _, _ = product_of_experts(marginals, precision_weights=[1.0, 1.0])
    displaced = joint_mean.copy()
    displaced[0] += 0.006
    observation_covariance = np.eye(6) * 1.0e-4

    def score(observed_pose: np.ndarray, *, broaden: bool):
        observed = relative_pose(frame, observed_pose)
        return joint_peak_normalized_pose_support(
            [
                (
                    name,
                    frame,
                    mean,
                    covariance,
                    covariance + observation_covariance if broaden else covariance,
                    observed,
                    1.0,
                )
                for name, mean, covariance in (
                    ("first", first_mean, first_covariance),
                    ("second", second_mean, second_covariance),
                )
            ],
            diagonalize=False,
        )

    strict = score(displaced, broaden=False)
    broadened = score(displaced, broaden=True)
    at_mean = score(joint_mean, broaden=True)
    assert broadened[0] > strict[0]
    assert at_mean[0] == pytest.approx(1.0)
    assert at_mean[1] == pytest.approx(at_mean[2], abs=1.0e-12)
    ...
    for demo_index in range(5):
        ...
        demonstrations.append(
            DynaMACDemonstration(
                ...
                name=f"boundary_{arm_id}_{demo_index}",
            )
        )
    policy = DynaMAC(
        DynaMACConfig(
...
def test_local_completion_reads_terminal_posterior_once() -> None:
    model, demonstrations = _make_model("single")
    boundary_id = BoundaryId("single", 0, 1)
    belief = _belief(model, demonstrations, tick=1, end_probability=0.6)
    guard = EntryGuard(
        {"single": model},
        "single",
        _config({"single": model}, cycles=1),
    )
    ...
    assert local.end_probability == pytest.approx(0.6)
    assert local.score == pytest.approx(0.6)
    assert local.done
    assert request.local_done
...
def test_local_done_and_guard_conditions_require_stable_cycles() -> None:
    ...
    assert first_local.score == pytest.approx(1.0)
...
def test_runtime_pose_variance_is_propagated_per_robot_endpoint_only() -> None:
    left_model, _ = _make_model("left")
    right_model, _ = _make_model("right")
    models = {"left": left_model, "right": right_model}
    guard = EntryGuard(
        models,
        "left",
        _config(models, cycles=1),
        robot_runtime_variance=4.0e-5,
    )

    assert guard._scene_runtime_covariance(
        FactorId("edge", "object", "target"), 6
    ) is None
    np.testing.assert_allclose(
        guard._scene_runtime_covariance(
            FactorId("edge", "left_ee", "right_ee"), 6
        ),
        np.eye(6) * 8.0e-5,
    )
    assert robot_pose_observation_covariance(4.0e-5, 0) is None


def test_unknown_own_relation_blocks_boundary_without_redefining_local_progress() -> None:
    model, demonstrations = _make_model("single")
    guard = EntryGuard(
    ...
    assert local.evidence_available
    assert local.done
...
def test_guard_relation_and_scene_fail_closed_and_reset_streak() -> None:
    ...
    relation = next(
        ...
        if condition_id.kind == ConditionKind.GUARD_RELATION
        and condition_id.token == "single/target"
    )
...
def test_cross_arm_relation_guard_uses_other_arm_current_belief() -> None:
    ...
    cross_result = next(
        ...
        for condition_id, result in blocked.condition_results.items()
        if condition_id.kind == ConditionKind.GUARD_RELATION
        and condition_id.token == cross_guard
    )
    ...
    # A progress-explanation miss remains a recovery trigger while a boundary
    # is waiting.  Runtime evidence may later make a legal re-entry available;
    # suppressing subsequent requests by boundary identity alone was rejected
    # by the full nominal regression because it removed recoverable attempts.
    ...
    assert repeated.reasons == ("left:no_plausible_state",)
...
def test_multi_arm_transaction_is_joint_or_asynchronous(transaction_group) -> None:
    ...
    models = {"left": left_model, "right": right_model}
    controllers = {
        arm: TSFExecutionController(model) for arm, model in models.items()
    }
...
def test_transition_preparation_requires_an_active_matching_link_guard() -> None:
    left_boundary_id = BoundaryId("left", 5, 6)
    right_boundary_id = BoundaryId("right", 5, 6)
    relation_key = "right/item"
    link_condition = RelationGuardDistribution(0.3, 0.7, "linked")
    left_boundary = SimpleNamespace(
        boundary_id=left_boundary_id,
        terminal_window=(StateId(5, 20),),
        relation_conditions={relation_key: link_condition},
    )
    right_boundary = SimpleNamespace(
        boundary_id=right_boundary_id,
        terminal_window=(StateId(5, 21),),
        relation_conditions={},
    )
    controller = object.__new__(MultiArmBoundaryController)
    controller.task_models = {
        "left": SimpleNamespace(boundaries={left_boundary_id: left_boundary}),
        "right": SimpleNamespace(boundaries={right_boundary_id: right_boundary}),
    }
    controller._preparations = {}
    ...
    right_request = SimpleNamespace(
        arm_id="right",
        boundary_id=right_boundary_id,
        condition_results={},
        preparation=preparation,
    )
    sources = {"right": StateId(5, 21)}

    # A nearby LINK event is only a candidate.  Without an active boundary
    # dependency it must not execute before the transition commits.
    controller._reconcile_preparations({"right": right_request}, sources)
    assert controller.preparations == {}

    # A separate arm may express the shared readiness condition.  The exact
    # receiver/object pair then authorizes the receiver's preparation.
    link_id = ConditionId(ConditionKind.GUARD_RELATION, "right", relation_key)
    left_request = SimpleNamespace(
        arm_id="left",
        boundary_id=left_boundary_id,
        condition_results={link_id: object()},
        preparation=None,
    )
    controller._reconcile_preparations(
        {"left": left_request, "right": right_request},
        {"left": StateId(5, 20), **sources},
    )
    assert controller.preparations == {"right": preparation}

    # Once the shared dependency is no longer active, a latched preparation
    # is removed instead of leaking into an unrelated boundary.
    controller._reconcile_preparations({"right": right_request}, sources)
    assert controller.preparations == {}
