...
def belief_model():
    ...
    return TSFTaskModelBuilder().build(policy, demonstrations), demonstrations
...
def test_simple_state_updater_is_one_hot_and_stage_guarded(belief_model) -> None:
    model, demonstrations = belief_model
    ...
    updater = NearestDemoStateUpdater(model)
    updater.reset(previous_observation=previous)
    belief = updater.update(
        current,
        executed_reference_state=StateId(0, 0),
    ...
    assert {state.skill_index for state in belief.local_candidates} == {0}
    assert sum(value == 1.0 for value in belief.progress.posterior.values()) == 1
    assert sum(belief.progress.posterior.values()) == pytest.approx(1.0)
...
def test_rejected_action_stream_is_not_reused_as_continuous_progress_evidence(
    belief_model,
    monkeypatch,
) -> None:
    model, demonstrations = belief_model
    ...
    monkeypatch.setattr(
        ...
        tuple(frame for frame in node.action_relevant_frames if frame != "object"),
    )
    monkeypatch.setattr(
        node,
    ...
    demo = demonstrations[0]
    displaced_object = demo.frames["object"][1].copy()
    displaced_object[0] += 1.0
    features = RuntimeFeatureBuilder().build(
        RuntimeObservation(
            tick=1,
            ee_pose=demo.ee_pose[1],
            frame_poses={"object": displaced_object},
            gripper_state=np.asarray([1.0]),
            previous_command_pose=demo.action_pose[0],
            previous_ee_pose=demo.ee_pose[0],
            tracking_reliability={"object": 1.0},
    ...
    assert "object" not in score.robot_frame_terms
    assert "object" not in score.robot_frame_weights


def test_relation_filter_has_shared_physical_semantics_without_static_false_link(
    belief_model,
) -> None:
    model, _ = belief_model
...
def test_invisible_or_unreliable_relation_is_unknown(belief_model) -> None:
    model, _ = belief_model
    builder = RuntimeFeatureBuilder()
    previous = observation(0, 0.0, 0.4, previous_ee_x=None)
...
def test_progress_prior_is_conditioned_on_the_action_reference(belief_model) -> None:
    model, _ = belief_model
...
def test_progress_posterior_can_realign_backward_and_forward(belief_model) -> None:
    model, _ = belief_model
...
def test_relation_mismatch_does_not_deform_external_candidate_motion_score(
    belief_model,
) -> None:
    model, demonstrations = belief_model
    demo = demonstrations[0]
    ...
    # The candidate's successful demonstrations define this frame as an
    # external action reference.  A contradictory online relation is handled
    # by the independent relation term/recovery path; it must not silently
    # move the candidate's robot-motion target.
    assert score.robot_frame_weights["object"] == pytest.approx(1.0)
    assert score.relation_state_compatibility == 0.0
    assert math.isfinite(score.robot_log_likelihood)


def test_runtime_robot_variance_models_execution_dispersion_without_losing_raw_audit(
    belief_model,
) -> None:
    model, demonstrations = belief_model
    demo = demonstrations[0]
    displaced = demo.ee_pose[1].copy()
    displaced[0] += 0.006
    runtime = RuntimeObservation(
        tick=1,
        ee_pose=displaced,
    ...
    strict = StateEvaluator(
        model,
        StateEvaluatorConfig(robot_runtime_variance=0.0),
    ).evaluate(StateId(0, 1), features, relations, mode_by_skill={0: 0})
    runtime_aware = StateEvaluator(
        model,
        StateEvaluatorConfig(robot_runtime_variance=1.0e-5),
    ).evaluate(StateId(0, 1), features, relations, mode_by_skill={0: 0})

    assert runtime_aware.robot_runtime_variance == pytest.approx(1.0e-5)
    assert (
        runtime_aware.robot_peak_normalized_compatibility
        > strict.robot_peak_normalized_compatibility
    )
    strict_audit = strict.robot_frame_gaussian_audits["object"][0]
    runtime_audit = runtime_aware.robot_frame_gaussian_audits["object"][0]
    assert runtime_audit.raw_log_likelihood == pytest.approx(
        strict_audit.raw_log_likelihood
    )
    assert runtime_audit.mahalanobis_squared == pytest.approx(
        strict_audit.mahalanobis_squared
    )
    assert runtime_audit.covariance_log_determinant == pytest.approx(
        strict_audit.covariance_log_determinant
    )
    assert runtime_audit.effective_mahalanobis_squared is not None
    assert (
        runtime_audit.effective_mahalanobis_squared
        < runtime_audit.mahalanobis_squared
    )


def test_unknown_relation_does_not_rescale_candidate_motion_evidence(
    belief_model,
) -> None:
    model, demonstrations = belief_model
    demo = demonstrations[0]
    ...
    assert visible_score.robot_frame_weights["object"] == pytest.approx(0.5)
...
def test_robot_progress_normalizes_by_the_executable_joint_poe_target(
    belief_model,
    monkeypatch,
) -> None:
    model, demonstrations = belief_model
    ...
    # mean, then observe the exact target of their executable joint PoE.
    monkeypatch.setattr(node, "selected_frames", ("object", "second"))
    monkeypatch.setattr(node, "mode_selected_frames", (("object", "second"),))
    monkeypatch.setattr(node, "action_relevant_frames", ("object", "second"))
    monkeypatch.setattr(
        node, "mode_action_relevant_frames", (("object", "second"),)
    )
    second_means = node.stream_means["object"].copy()
    second_means[0, 0] += 0.03
    monkeypatch.setitem(node.stream_means, "second", second_means)
    monkeypatch.setitem(
        node.stream_covariances,
        "second",
        node.stream_covariances["object"].copy(),
    )
    second_frame = demo.frames["object"][1].copy()
    marginals = [
        transform_marginal(
            "object",
            demo.frames["object"][1],
            node.stream_means["object"][0],
            node.stream_covariances["object"][0],
        ),
        transform_marginal(
            "second",
            second_frame,
            node.stream_means["second"][0],
            node.stream_covariances["second"][0],
        ),
    ]
    joint_mean, _, _ = product_of_experts(
        marginals, precision_weights=[0.999, 0.999]
    )
    runtime = RuntimeObservation(
        tick=1,
        ee_pose=joint_mean,
    ...
    assert score.robot_peak_normalized_compatibility == pytest.approx(1.0)
    # The candidate is represented by one fused action distribution, so its
    # mean has unit peak support even when the local experts disagree.
    assert score.robot_attainable_peak_log_support == pytest.approx(0.0)
    assert score.robot_log_support == pytest.approx(0.0)
...
def test_node_and_edge_scene_factors_use_the_unified_observation(belief_model) -> None:
    model, _ = belief_model
...
def test_belief_updater_is_single_pass_and_auditable(belief_model) -> None:
    model, demonstrations = belief_model
    demo = demonstrations[0]
    previous = RuntimeObservation(
        tick=0,
...
def test_tracked_phase_two_config_matches_code_defaults() -> None:
    path = Path("configs/tsf_inference.json")
