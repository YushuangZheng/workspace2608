"""A6 Horizon-3 queue using the frozen queue with an E4 episode entry point."""
...
class HorizonDynamicQueue(launch.DynamicQueue):
    """Change only the child module used for E4 per-stage episodes."""
    ...
    def _launch(self, row: dict[str, Any], lane_index: int) -> None:
        ...
        command = [
            ...
            str(self.sim_python),
            "-m",
            "evaluations.iclr2027.runners.a6_horizon_episode",
            "--manifest",
            str(self.manifest),
            "--episode-id",
            str(row["episode_id"]),
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.limit_per_task is not None:
        ...
        rows = limited
    if not rows:
        raise ValueError("no Horizon-3 manifest rows selected")
    if any(row.get("event_schedule") != "one_eligible_event_per_interaction_stage" for row in rows):
        raise ValueError("A6 Horizon queue accepts only per-stage rows")
    queue = HorizonDynamicQueue(
        rows,
    ...
    metadata["episode_module"] = "evaluations.iclr2027.runners.a6_horizon_episode"
