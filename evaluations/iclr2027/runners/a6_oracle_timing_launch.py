"""A6 diagnostic queue using the isolated oracle-timing episode entry point."""
...
class OracleTimingDynamicQueue(launch.DynamicQueue):
    """Change only the child module used by the diagnostic queue."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.limit_per_task is not None:
        ...
        rows = limited
    if not rows:
        raise ValueError("no oracle timing diagnostic rows selected")
    method = launch.load_method_spec(args.method)
    if method.method_id != "m5_full":
        raise ValueError("oracle timing queue requires the frozen m5_full method")
    queue = OracleTimingDynamicQueue(
        rows,
    ...
    metadata["episode_module"] = EPISODE_MODULE
    metadata["diagnostic_only"] = True
    metadata["oracle_inputs"] = ["violation_onset_cycle"]
