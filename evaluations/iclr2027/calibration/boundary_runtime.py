"""Calibrate boundary thresholds from normal control-tick replay."""
...
from evaluations.iclr2027.calibration.demo_replay import (
    BELIEF_CONFIG_PATH,
...
SCHEMA = "essay2608-boundary-calibration-config-v2"
...
def _read_config(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict) or set(value) != required:
        raise ValueError("边界标定配置字段不完整或包含未知字段")
    if value["schema"] != SCHEMA:
        raise ValueError("边界标定配置 schema 不匹配")
    if not value["tasks"] or not value["demonstration_indices"]:
        raise ValueError("边界标定任务和示范索引不能为空")
...
def run(
    ...
    output: Path,
    *,
    case_loader: Callable[[str, int], Sequence[Any]],
    include_skill_entry_cycles: bool = False,
) -> None:
    ...
    for task in config["tasks"]:
        print(f"[boundary-calibration] loading {task}", flush=True)
        cases = list(case_loader(task, maximum_demo + 1))
        runtime_config = _base_runtime_config(cases, config)
        for demonstration in config["demonstration_indices"]:
            print(
                f"[boundary-calibration] replay {task} demo {demonstration}",
                flush=True,
            )
            all_rows.extend(
                _replay_task_demo(
    ...
    for task in config["tasks"]:
        print(f"[boundary-calibration] validating {task}", flush=True)
        cases = list(case_loader(task, maximum_demo + 1))
    ...
    print(f"[boundary-calibration] wrote {output}", flush=True)
