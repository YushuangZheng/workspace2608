...
为避免早期边界误差级联，进入每个新技能时假设上一边界已经正常提交：进度初始化到该技能首状态，同时携带此前已确认关系后验和稳定决策。固定离线回放使用产生当前观测的示范动作目标 StateId；控制器查询仅作审计，不反馈到未执行的轨迹。
...
```bash
python -m evaluations.development.phase4_boundary_readiness.run \
  --config evaluations/development/phase4_boundary_readiness/config.json \
  --output evaluations/development/phase4_boundary_readiness/results/v7_reproduction
```
