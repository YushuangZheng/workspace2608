...
class TransitionTransactionCoordinator:
    ...
    def __init__(
        ...
        if set(task_models) != set(controllers):
            raise ValueError("事务协调器的任务模型与执行控制器必须覆盖同一组机械臂")
    ...
    def commit(
        ...
        # Validate the complete batch before changing any cursor.  Once this
        # loop succeeds, each commit below is a deterministic in-memory write.
