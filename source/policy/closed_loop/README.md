...
正式 LINK 的本次执行确认允许使用阶段一为该 occurrence 学到的完整 `linked_entry_states` 自然运动区间，不再使用全局固定状态数。该有限区间从全部对齐正常示范均已发出因果闭爪的状态开始，到首次稳定运动学 LINK 证据满足最短持续要求时结束；正式 LINK 后直至对应 UNLINK 的长期关系来源只由 `link_origins` 表示，不能反过来延长确认宽限。确认区间只有从正式 LINK 的直接前驱沿拓扑进入时才激活，reset、恢复重入或任意跳转到 linked 状态不能重新取得宽限。事件直接前驱上的 pre-LINK external/Unknown 不能阻断建立关系所必需的后继命令；进入区间后，相应物理流保持零权重 `DEFER`，先提交抓取并执行有界自然携带，再由后续运动证据确认。当前查询状态可以比 beta 领先一个周期，因此区间内的实体即使不是当前 Eq.6 动作流也会被加入零权重关系监控，正式 occurrence 自身提供 linked 目标语义，但该实体不会重新进入动作 PoE。末入口状态在 beta 尚未到达时继续完成自然动作；若该末状态本周期已给出可用的 linked 方向运动响应而持续后验尚未稳定，可执行仅一个直接拓扑后继作为最后确认动作。该路径不直接确认 linked、不跨越第二个后继；下一观测未由原滤波器稳定确认则进入正常失配/恢复。beta 确认末状态且不满足该有限条件后才结束宽限；一旦确认 linked，后续掉落也不能重新进入宽限。
