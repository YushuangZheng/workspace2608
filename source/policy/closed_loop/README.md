...
双臂构建把 Eq.6 选中的对侧末端视为候选而非既成执行依赖。逐 `(skill, mode)` 只有两类证据可以保留该连续流：本臂活 `link_origin` 与对臂正式 LINK/`LINK_PENDING` 指向同一共享或交接实体；或者该模态没有普通物理任务参考系、对侧末端是唯一非虚拟目标，并具有稳定非零定向相对几何。普通物体/目标流已经解释动作且不存在关系路径时，对侧末端只因示范同步被选中，便从闭环 sidecar 的 PoE、进度轨迹和本地完成目标中删除。冻结 V4 checkpoint 不改写，筛选记录保存在 `builder_config.peer_execution_dependencies`；当前 sidecar schema v5 逐状态保存筛选后的 `selected_frames/mode_selected_frames`，加载时不会恢复成旧 Eq.6 全集。纯同步不产生动作依赖、边界依赖或事务；真实就绪仍由统一关系/场景守卫表达。

所有 Eq.6 物理候选还逐 `(skill, mode)` 执行通用离线动作相关性 LODO 筛选。每折用其余示范拟合技能入口虚拟帧基线与“基线＋候选” PoE，并在留出示范上比较三维位置预测误差。普通附加候选需至少 4/5 折改善超过 `0.1 mm`，且平均增益也超过该值；Eq.6 最高分候选作为原对象中心假设的主锚保留，并保证任一原活动状态至少有一个动作专家。结果只写入 `action_relevant_frames/mode_action_relevant_frames`；原 `selected_frames`、进度、关系、场景/边界和恢复证据均不删除。留一计算只在离线构建时进行，完整折结果和统一折分保存在 `builder_config`。
...
`ClosedLoopTaskModel.save()` 使用 sidecar schema v5 分开保存 Eq.6 候选流与动作相关性子集，并保存关系、Pending 事件、场景、边界和恢复统计；schema v3/v4 仍可只读加载，其动作相关性默认等于当时的候选流。原有流均值、协方差和夹爪模型继续从绑定的 DynaMAC checkpoint 读取。
...
`FrameRoleRouter` 对当前状态和 mode 的 Eq.6 候选流分配动作角色，但正常 PoE 只接收同时属于离线 `action_relevant_frames` 且当前角色为 `EXECUTE` 的流。未通过动作相关性的 external/external 候选是零权重 `DEFER`，但其关系、进度和恢复语义仍保留。路由器同时继续监控当前进度具有正式 `link_origin` 的物理关系，即使对应专家已不再被当前 mode 选择。虚拟技能帧只在属于动作相关性子集时执行，且不进入关系判断；物理流根据进度后验加权的期望关系、在线关系后验和跟踪可靠性分为：
...
- `DEFER`：实际关系 Unknown 时不把不确定性冒充为 external/linked。对具有动作权限、尚无可信历史、可见可靠且示范期望与二元后验都支持 external 的流，允许使用 $q(\mathrm{external})$ 安全降权产生自然激励；对仅保留作状态解释、没有动作权限且期望 external 的候选，保持零权重并把 Unknown 作为软证据，而不让它冻结真正的执行流。必要 linked、正式关系监控或已有可信关系失效时仍严格阻断相关推进。进度加权的期望关系暂时模糊时也不产生虚假失配或单独阻断进度。
...
正式 LINK 的本次执行确认允许使用阶段一为该 occurrence 学到的完整 `linked_entry_states` 自然运动区间，不再使用全局固定状态数。该有限区间从全部对齐正常示范均已发出因果闭爪的状态开始，到首次稳定运动学 LINK 证据满足最短持续要求时结束；正式 LINK 后直至对应 UNLINK 的长期关系来源只由 `link_origins` 表示，不能反过来延长确认宽限。确认区间只有从正式 LINK 的直接前驱沿拓扑进入时才激活，reset、恢复重入或任意跳转到 linked 状态不能重新取得宽限。事件直接前驱上的 pre-LINK external/Unknown 不能阻断建立关系所必需的后继命令；进入区间后，相应物理流保持零权重 `DEFER`，先提交抓取并执行有界自然携带，再由后续运动证据确认。若入口动作通过跨技能边界桥接提交，桥接周期保留源末端连续动作和流权重，同时在因果游标切到目标入口前预览入口并只合并正式关系事件生命周期，避免入口窗口因源快照覆盖而丢失。当前查询状态可以比 beta 领先一个周期，因此区间内的实体即使不是当前 Eq.6 动作流也会被加入零权重关系监控，正式 occurrence 自身提供 linked 目标语义，但该实体不会重新进入动作 PoE。末入口状态在 beta 尚未到达时继续完成自然动作；若该末状态本周期已给出可用的 linked 方向运动响应而持续后验尚未稳定，可执行仅一个直接拓扑后继作为最后确认动作。该路径不直接确认 linked、不跨越第二个后继；下一观测未由原滤波器稳定确认则进入正常失配/恢复。beta 确认末状态且不满足该有限条件后才结束宽限；一旦确认 linked，后续掉落也不能重新进入宽限。
...
`P_end` 是本臂进度后验落在完整终止窗口的概率；`L_goal` 是当前 mode 全部最终目标流相对于联合可达 PoE 峰值归一后的完整状态支持，不要求末端精确落在单一动作均值；`C_own-rel` 是本臂必要关系后验与离线终止关系分布内积的最小值，空集合为1。任一本臂必要关系 Unknown、不可见或不可靠时，本周期 `LocalDone=False`，但不把它直接升级为恢复异常。
...
`VERIFY_LINK` 接受阶段三或阶段四产生、并引用确切 LINK occurrence 的必要 linked 请求。请求既可以来自离线 `LinkPendingCandidate`，也可以来自本次执行已因果穿过、但在完整自然确认区间结束后仍未得到稳定关系结论的正式 LINK。后者只有在参考系可见可靠、当前激励不足且没有新的可靠 external 反证时才进入验证；新鲜且信息充分的 external 反证仍直接进入普通 LINK 恢复，不能被验证分支掩盖。边界准备中的 Pending 可在源 StateId 尚未提交时按确切事件标识路由，但必须先从真实观测确认准备闭爪已发生，不允许一边开爪一边探测。验证期间 beta/StateId 仍冻结；仅对请求指定的 arm-frame，原关系滤波器使用请求保存的事件关系上下文：Pending 使用 `candidate_state`，正式 LINK 使用其自然确认区间末状态；其他关系仍按冻结 beta 混合。该上下文不修改进度、不提供物理证据，也不新增关系状态。其他入口条件仍是关系尚未可靠确认、参考系成对可用且跟踪可靠、信息权重不足。控制器保持入口朝向和夹爪命令，按最近 TASK 实际末端轨迹估计接近方向，以固定速度反向探测；刚进入模式时属于上一条 TASK 指令的运动不计为验证证据，只累计验证动作发出后下一周期观测到的末端—参考系成对响应。当前至少需要3个可靠动作响应和既有最小探测运动，并用窗口共动残差比 `sqrt(sum(||delta_p_f-delta_p_e||^2)/sum(||delta_p_e||^2))` 的统一0.85阈值给出响应方向；只有该方向与原二元关系滤波器当前非 Unknown、有效信息决策一致时，关系才稳定成立。窗口不创建第三种关系状态，也不替代原滤波器。稳定关系、探测时间上限或安全约束任一成立便停止外移；时间上限只计算探测阶段，之后仍按记录的实际路径反向返回，仅当返回路径本身不安全时输出结构化失败。返回后统一回到 TASK；若已稳定确认 linked/external，则把稳定周期的原始二元后验精确提交回同一个在线关系滤波状态，不做 one-hot 化或固定置信度增强，避免返回运动的短时残差抹掉有效确认。稳定验证为 linked 时，只有 Pending 才在当前 episode 激活其恢复模板并建立运行时来源；正式 LINK 的来源已经由离线事件定义。验证为 external/Unknown 时不激活 Pending。若边界准备后稳定判为 external，恢复意图保留同一 Pending 事件标识并只读使用其未激活恢复模板，直到恢复获得 linked 证据才激活来源。同一事件只有关系状态、任务状态或抓取事件变化后才能再次验证。
