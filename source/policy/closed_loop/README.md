...
闭环策略的 RLBench 执行配置为 `stage6_hybrid_cartesian_executor_v16` / `rlbench-stage6-hybrid-cartesian-continuation-v18`：优先使用 CoppeliaSim 当前关节种子伪逆，再依次使用 bounded TRAC-IK Distance、有界 SE(3) 小步延续、碰撞感知优先且必要时放宽的采样 IK和碰撞感知线性路径。远目标可直接进入碰撞感知 RRTConnect；近目标首次求解不调用全局规划，只有同一策略目标被真实物理反馈确认停滞并耗尽本地求解层级后，才在碰撞放宽直线之前尝试碰撞感知 RRTConnect。若合法关节目标执行后对同一个原始笛卡尔目标没有真实改善，也按该顺序执行一次有界求解器升级；升级中的采样候选必须满足与 bounded TRAC-IK 相同的当前关节连续性上限，避免末端近似不变却跳到远距离逆解分支。普通“全部局部 IK 初始不可行”的全局采样后备仍保留，并选择离当前关节状态最近的合法候选。任一备用解产生真实进展后立即返回策略，下一闭环周期重新观测和求解，不在一个周期内隐藏地推进多个策略状态。每段移动后重新观测真实末端，内部子目标不提交为策略状态；bounded TRAC 请求和执行后物理完成使用 0.5 mm / 0.05°，外层策略目标接受使用 1 mm / 0.1°，只有原始目标进入外层包络才报告 `reached`。真实改善、物理停止和全链耗尽分别返回可区分结果；2 mm / 1° 仅是外部运动链 FK 对齐审计上限。`progressed/stopped` 只表示本次物理前缀，不能把绝对策略目标、正常进度、恢复路点或固定时钟动作提交为已完成；下一周期必须重新观测并重试同一目标。夹爪命令只在相应机械臂 `reached` 后发出，双臂夹爪则在两臂都到达后原子执行。双臂先完成全部候选准备再推进共享物理时钟。执行器不读取任务名或 StateId，可随 benchmark 替换而不改变阶段一至五算法。
