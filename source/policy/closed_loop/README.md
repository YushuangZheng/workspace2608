...
本目录按照《新方法代码开发计划》组织关系—进度信念驱动方法。阶段一离线任务模型、阶段二关系—进度联合信念推断、阶段三闭环正常执行控制、阶段四对象中心入口守卫与事务提交，以及阶段五主动关系验证、统一关系恢复与任务重入均已实现。阶段六的环境无关顶层策略、序列化、逐周期诊断和 RLBench 适配已经接入，正常任务全量回归与故障消融尚未验收完成。
...
正式 LINK 的本次执行确认允许使用阶段一为该 occurrence 学到的完整 `linked_entry_states` 自然运动区间，不再使用全局固定状态数。确认前相应流为零权重 `DEFER`；可靠且信息充分的 external 反证立即退出宽限，一旦确认 linked 后也不能因后续掉落重新进入宽限。
...
运行参数按任务保存在 `configs/closed_loop_boundary/`。`theta_local` 和 `H` 只由五条正常示范的原始 RLBench 控制周期回放标定，并且只读取本地完成分数，不用守卫是否已经放行筛选本地正样本。RLBench 控制周期为0.05秒，39个边界使用 `H=2`（0.10秒），WipeDesk `1→2` 使用 `H=4`（0.20秒）。当前正式复核覆盖40个边界、200个边界×示范实例，边界前提前放行为0；正常末端保持后195次正确放行，Handover 左臂 `5→6` 的5条示范均在本地完成后因右臂尚未 linked 而正确等待；LiftTray `1→2` 的真实事务组在5/5条正常示范中双方共同就绪。12个未变单臂边界的结果保存在 `evaluations/phase4_boundary_calibration/results/v3`，28个重建双臂边界及其控制周期轨迹、决策表和校验和保存在 `evaluations/peer_execution_dependency/results/v1`。
...
`VERIFY_LINK` 只接受阶段三或阶段四产生、且引用当前 `LinkPendingCandidate` 的必要 linked 请求。入口必须满足关系尚未可靠确认、参考系成对可用且跟踪可靠、信息权重不足。控制器保持入口朝向和夹爪命令，按最近 TASK 实际末端轨迹估计接近方向，以固定速度反向探测；稳定关系判定、探测时间上限或安全约束任一成立便停止外移。时间上限只计算探测阶段，之后仍按记录的实际路径反向返回；仅当返回路径本身不安全时输出结构化失败。返回后统一回到 TASK。稳定验证为 linked 时只在当前 episode 激活 Pending 恢复模板并建立运行时来源，不修改离线任务模型；验证为 external/Unknown 时不激活。同一事件只有关系状态、任务状态或抓取事件变化后才能再次验证。

`RECOVERY` 将可靠二元关系失配转换为事件级 `LINK(f)` 或 `UNLINK(f)` 目标。同一机械臂的所有 UNLINK 先于 LINK，以先释放夹爪资源。LINK 从当前正式 `link_origin`、当前 episode 已验证的 Pending 来源，或“已到达同一 Pending occurrence 且必要 linked 守卫稳定判为 external”的未激活修复模板解析锚点；第三种来源只有在恢复后获得有效 linked 证据时才转为 episode 运行时来源。跨臂定向守卫在对臂尚未到达 Pending 时仍只是等待，不会误触发恢复。局部轨迹依据参考系当前世界位姿重新实例化，并对协方差统一加 `lambda_rec I`；UNLINK 统一执行打开夹爪、局部脱离和 external 验证。只有支持目标关系的有效运动证据才可完成关系目标，反方向或无信息证据不能替代确认。每个路点、关系验证、单目标尝试、整个恢复及重入等待均有明确周期上限和结构化失败。

关系目标完成后只在事件元数据提供的合法状态中比较当前完整机器人轨迹、稀疏场景和关系解释度；无关系目标的 `NO_PLAUSIBLE_STATE` 恢复可搜索全任务，但同技能之外只允许经入口守卫放行的相邻下一技能，禁止后退或跨多技能直接跳转。正常进度评分仍使用原始示范轨迹协方差；只有恢复后的重入机器人轨迹项复用既有 `lambda_rec`，按 `Sigma_reentry=Sigma+lambda_rec I` 评分，使恢复动作分布与其完成判定具有相同精度口径。场景、关系、候选范围、入口守卫和兼容度阈值不变。重入选择、进度后验 one-hot 重置、执行 `reference_state` 设置和返回 TASK 由管理器原子完成，不恢复旧故障时钟。
...
RLBench 专用代码位于 `integrations/rlbench/rlbench_closed_loop/`，负责低维观测适配、双臂联合快照、策略进程协议和命令格式；benchmark 执行器位于 `integrations/rlbench/rlbench_dynamac/core/runtime.py` 与 `trac_ik.py`。现有 `direct_evaluate` 通过 `policy_type=closed_loop_multistream` 复用同一评测集和结果 schema，同时保留 `policy_type=dynamac` 的冻结 V4 执行器对照路径。跨技能事务提交后，同周期动作沿用源技能最终连续位姿目标并读取已提交入口状态的夹爪命令，下一周期进度质量原子投影到入口状态；新捕获的虚拟技能帧立即作为可靠内部固定参考参与查询。

闭环策略的 RLBench 执行配置为 `stage6_hybrid_cartesian_executor_v10` / `rlbench-stage6-hybrid-cartesian-continuation-v12`：优先使用 CoppeliaSim 当前关节种子伪逆，只有它未产生合法候选时才调用 bounded TRAC-IK Distance；全目标仍不可解时继续使用有界 SE(3) 小步延续、碰撞感知优先且必要时放宽的采样/线性路径，以及仅用于远目标的碰撞感知 RRTConnect。每段移动后重新观测真实末端并重建求解，内部子目标不提交为策略状态；bounded TRAC 请求、执行后物理完成和控制接受统一使用 0.5 mm / 0.05°，只有原始末端目标进入该包络才报告 `reached`，真实改善、物理停止和全链耗尽分别返回可区分结果。2 mm / 1° 仅是外部运动链 FK 对齐审计上限。双臂先完成全部候选准备再推进共享物理时钟。执行器不因一次合法命令的物理停滞而对同一目标立即切换求解器反复震荡，不读取任务名或 StateId，可随 benchmark 替换而不改变阶段一至五算法。
...
阶段六仍处于开发中：当前统一精度下，封存正常 Handover seed `2608000000`、LiftTray seed `2608000001` 和 StackWine seed `2608000000` 分别在494、106和168周期成功，均为0 InvalidAction、0全IK链耗尽和0控制预算耗尽。Handover 右臂 `4→5` 的双臂末端硬场景条件在 tick 212 连续满足后正常放行，没有放宽守卫或建立任务专用事务。已知困难 LiftTray seed `2608000000` 的后续抓取/关系与重入残余仍需独立诊断；正常全任务回归与故障消融尚未完成，因此阶段六仍不记为已验收。
