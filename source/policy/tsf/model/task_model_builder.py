"""Build the TSF task model from the same successful demonstrations."""
...
from .task_model import TSFTaskModel, StateNode

Array = np.ndarray


@dataclass(frozen=True)
class TSFTaskModelConfig:
    """Task-model statistics not contained in the base-policy checkpoint."""
    ...
    def __post_init__(self) -> None:
        ...
        if any(not 0.0 <= value <= 1.0 for value in probabilities):
            raise ValueError("任务模型概率阈值必须位于 [0,1]")
        ...
        if any(
            ...
            raise ValueError("任务模型窗口长度必须为正整数")
...
class _AlignedSkillData:
    ...
    # TSFTaskModel; offline evaluators use it to reconstruct the action
...
def _relation_prior(
    scale: float, config: TSFTaskModelConfig, tau_m: float
) -> Array:
...
class TSFTaskModelBuilder:
    """Build one arm's unified model without refitting its base DynaMAC."""

    def __init__(
        self, config: TSFTaskModelConfig = TSFTaskModelConfig()
    ) -> None:
        self.config = config

    @staticmethod
    def _relation_pose(data: _AlignedSkillData, frame: str) -> Array:
        """Return an observed entity pose without granting action authority."""

        if frame in data.frames:
            return data.frames[frame]
        if frame in data.scene_entity_poses:
            return data.scene_entity_poses[frame]
        raise KeyError(f"关系实体 {frame!r} 不在动作参考或场景观测中")
    ...
    def _candidate_factors(
        ...
        window = tuple(
            self._window_indices(skill.duration, state_id.local_index, radius)
        )
        # Scene state complements robot pose and robot--frame relations; it
        # must not encode the same degree of freedom a second time.  An edge
        # touching a frame that is robot-linked anywhere in its fitting window
        # is therefore relation-redundant and is not a scene candidate.  Node
        # configurations remain eligible because they represent internal
        # entity state that no robot--frame relation exposes.
        action_references = tuple(
            name
            for name in states[state_id].mode_action_relevant_frames[mode]
            if self._is_scene_entity(name)
            and all(
                states[StateId(state_id.skill_index, index)].demo_relation_priors[
                    name
                ][mode, 1]
                < self.config.relation_link_threshold
                - self.config.relation_epsilon
                for index in window
            )
        )
        return self._factors_for_references(
            ...
            scene_entities=tuple(
                sorted(
                    set(data.scene_entity_poses).union(data.entity_configurations)
                )
            ),
        )
    ...
    def _build_states(
        ...
        relation_frames = getattr(
            self,
            "_state_relation_frames",
            tuple(policy.frame_names),
        )
        ...
        for skill_index, skill in enumerate(policy.skills):
            for local_index in range(skill.duration):
                ...
                states[state_id] = StateNode(
                    ...
                    # The builder first materializes the complete task-parameter
                    # candidate bank, then replaces this staging mask with the
                    # offline LODO result in _add_action_stream_relevance.
                    action_relevant_frames=tuple(skill.selected_frames),
    ...
    def _add_action_stream_relevance(
        ...
        """Remove only stably harmful base-policy candidates at skill level.

        Each fold fits the current skill's entry-frame motion baseline and one
        candidate from the other demonstrations.  A candidate is harmful only
        when adding it increases held-out Cartesian target error both on
        average and in the median fold.  Neutral, redundant and weakly useful
        streams are retained.  The decision is shared by the complete skill,
        so this screen cannot become a second state-wise progress router.
        The original candidate bank remains available for relation events,
        relation guards and recovery.
        """
            ...
            for mode in range(len(skill.mode_priors)):
                ...
                for frame in candidates:
                    ...
                    stably_harmful = bool(
                        ...
                        and median_gain < -self.config.action_relevance_min_loo_gain
                    )
                    keep = not stably_harmful
                    if keep:
                        retained.add(frame)
                    records[frame] = {
                        "fold_gains": gains,
                        ...
                        "task_parameter_relevance_score": float(
                            skill.selection_scores.get(frame, 0.0)
                        ),
                        "stably_harmful": stably_harmful,
                        "retained": keep,
                        "reason": (
                            ...
                            if is_baseline
                                ...
                                if not sufficiently_observed
                                    ...
                                    "stable_harmful_marginal_gain"
                                    if stably_harmful
                                    else "not_stably_harmful_keep"
                                )
                            )
                        ),
                    }
                ...
                audit.append(
                    {
                        ...
                        "mode": mode,
                        "comparison": "skill_entry_baseline_plus_one_candidate",
    ...
    def _joint_relation_evidence_sequence(
        ...
        frame_pose = np.concatenate(
            [
                self._relation_pose(aligned[index], frame)[members]
        ...
        scales = np.asarray(
            geometric_mean_standard_deviation(
                covariance,
                position_weight=policy.config.relation_position_weight,
                rotation_weight=policy.config.relation_rotation_weight,
            ),
            dtype=np.float64,
        )
        ...
        weights = np.asarray(
            [policy.config.relation_position_weight] * 3
            + [policy.config.relation_rotation_weight] * 3,
            dtype=np.float64,
        )
    ...
    def _build_relation_events(
        ...
        relation_frames: Sequence[str] | None = None,
    ) -> tuple[
        ...
        def unresolved_link_candidates(
            ...
            for index in sorted(set(collective_closes)):
                ...
                # A LINK_PENDING occurrence represents the entity that the
                # successful demonstrations consistently close on.  GMSD by
                # itself cannot establish that identity: another stationary
                # object can have an equally repeatable pose at the same
                # gripper transition and would then become a spurious LINK
                # target.  Conversely, the actual grasped entity can have a
                # wider cross-demonstration pose distribution while the short
                # post-close interval contains no motion with which to confirm
                # co-motion.  Use the already available object-centric scene
                # geometry to identify the directly approached physical
                # frame.  The comparison must include every observable task
                # entity, not only the recoverable subset.  Otherwise a lone
                # recoverable object becomes the nearest candidate by
                # construction even when the arm is actually closing near an
                # environmental fixture.  This adds no task label or runtime
                # variable: the candidate must be the unique nearest physical
                # frame in a cross-demonstration-supported fraction of the
                # same closing observations, and only then may a recoverable
                # frame become LINK_PENDING.
                target_skill = int(
                    np.searchsorted(skill_offsets[1:], index, side="right")
                )
                target_local = index - int(skill_offsets[target_skill])
                sample_data = aligned[target_skill]
                direct_support = 0
                for demonstration in members:
                    ee_position = sample_data.ee_pose[
                        int(demonstration), target_local, :3
                    ]
                    distances = sorted(
                        (
                            float(
                                np.linalg.norm(
                                    self._relation_pose(sample_data, candidate_frame)[
                                        int(demonstration), target_local, :3
                                    ]
                                    - ee_position
                                )
                        ...
                        for candidate_frame in relation_frames
                        if self._is_scene_entity(candidate_frame)
                    )
                    best_distance, best_frame = distances[0]
                    uniquely_nearest = bool(
                        len(distances) == 1
                        or best_distance + self.config.relation_epsilon
                        < distances[1][0]
                    )
                    if uniquely_nearest and best_frame == frame:
                        direct_support += 1
                if (
                    direct_support / float(len(members))
                    + self.config.relation_epsilon
                    < self.config.relation_event_support
                ):
                    continue
                candidates.append(("link", index - global_start))
            return candidates

        def next_collective_open(
            members: Array,
            event_global_index: int,
        ) -> int | None:
            """Return the first aligned causal opening after one Pending close."""

            commands = np.mean(
                np.asarray(joint_gripper, dtype=np.float64)[members],
                axis=2,
            )
            positions = []
            for demonstration in commands:
                openings = np.flatnonzero(np.diff(demonstration) > 1.0e-9) + 1
                later = openings[openings > event_global_index]
                if not len(later):
                    return None
                positions.append(int(later[0]))
            if (
        ...
        def fit_link_anchor_template(
            ...
            for state in anchor_states:
                ...
                local_samples = [
                    relative_pose(
                        self._relation_pose(sample_data, frame)[
                            demonstration, state.local_index
                        ],
        ...
        for skill_index, skill in enumerate(policy.skills):
            for mode in range(len(skill.mode_priors)):
                members = _mode_members(skill, mode)
                for frame in sorted(recoverable_frames):
                    ...
                    # the gripper and object-centric scene context identify a
                    # direct recoverable entity but the post-close segment has
                    # no informative motion.  This is event metadata only: it
                    # does not create a LINK anchor, propagate link_origin, or
                    # claim that linked is true before online verification.
                    _, pending_observable = pending_evidence(
                        frame, members
                    )
                    ...
                    if len(members) >= 2:
                        for held_out in members:
                            training = members[members != held_out]
                            _, fold_observable = pending_evidence(
                                frame, training
                            )
                    ...
                    for _, local_index in sorted(
                        ...
                        pending_links[event_id] = LinkPendingCandidate(
                            ...
                            release_state=(
                                None
                                if (
                                    release_index := next_collective_open(
                                        members,
                                        global_start + local_index,
                                    )
                                )
                                is None
                                else state_from_global(release_index)
                            ),
                    ...
                    for (
                        ...
                        if transition == "link":
                            ...
                            local = _relative_batch(
                                self._relation_pose(target_data, frame)[
    ...
    def _reconcile_confirmed_relation_paths(
        ...
        become a control-time linked expectation.  A Pending-only occurrence
        remains a soft linked hypothesis until a later LINK, LINK_PENDING, or
        UNLINK for the same arm--frame pair replaces it.  It never gains a
        formal origin before online verification.
        """
        ...
        # Store the lifecycle end per demonstration member.  Skill-local
        # mixture partitions may change after the close, so keying the end by
        # the exact source-mode tuple would silently lose the release when a
        # later skill repartitions the same demonstrations.
        pending_ends: dict[tuple[int, str, int], int] = {}
        for event_id, candidate in pending_links.items():
            ...
            if candidate.release_state is not None:
                end = global_index(candidate.release_state)
                for demonstration in members:
                    pending_ends[
                        (int(demonstration), event_id.frame_id, start)
                    ] = end
            for demonstration in members:
            ...
            for mode in range(len(skill.mode_priors)):
                ...
                for frame in relation_frames:
                    ...
                    if new_pending:
                        ...
                        pending_active = True
                    member_ends = [
                        pending_ends.get((member, frame, last_start))
                        for member in mode_members
                    ]
                    pending_end = (
                        max(int(value) for value in member_ends if value is not None)
                        if member_ends and all(value is not None for value in member_ends)
                        else None
                    )
                    if (
                        pending_active
                        and pending_end is not None
                        and state_global_index >= pending_end
                    ):
                        pending_active = False
                    # The event identity, rather than incidental covariance at
                    # every following sample, owns the relation lifecycle.  A
                    # later accepted formal transition replaces this Pending
                    # occurrence only after every demonstration member of the
                    # active mode has reached that transition.
                    if (
                        pending_active
                        and state_global_index > last_start
                        and tracks
                        and all(track is not None for track in tracks)
                        and all(
                            any(
                                last_start < index <= state_global_index
                                for index, _, _ in track
                            )
                            for track in tracks
                        ...
                        pending_active = False
                    pending_hypothesis = pending_active
                    ...
                    # normal-demo expectation nearly impossible for inference
        ...
        *,
        ...
        for mode in range(len(skill.mode_priors)):
            ...
            if require_discrimination:
                if start <= 0 or len(members) < 3:
                    return None
                for held_out in members:
                    training = members[members != held_out]
                    training_window = values[training, start : skill.duration]
                    training_samples = np.repeat(
                        training_window,
                        time_repeats,
                        axis=1,
                    ).reshape(-1, values.shape[-1])
                    fold_distribution = self._fit_factor(
                        training_samples,
                        policy,
                        factor_id,
                    )
                    positives = np.asarray(
                        [
                            fold_distribution.compatibility(value)
                            for value in values[held_out, start : skill.duration]
                        ],
                        dtype=np.float64,
                    )
                    ...
                    fold_accuracies.append(
                        float(
                            np.mean(comparisons > 0.0)
                            + 0.5 * np.mean(comparisons == 0.0)
                        )
                    )
        if require_discrimination and (
            min(fold_gains, default=float("-inf"))
            <= self.config.scene_min_loo_gain
            or min(fold_accuracies, default=0.0)
            < self.config.scene_min_loo_accuracy
        ):
            return None
        ...
        distribution = self._fit_factor(
            ...
            observability=1.0,
            stable_fraction=stable_fraction,
            loo_gain=(min(fold_gains) if fold_gains else 0.0),
            loo_accuracy=(
                min(stable_fraction, min(fold_accuracies))
                if fold_accuracies
        ...
        *,
        ...
        for mode in range(len(skill.mode_priors)):
            ...
            for held_out in members:
                ...
                for offset, local_index in enumerate(range(start, skill.duration)):
                    local = _relative_batch(
                        self._relation_pose(data, frame)[training, local_index],
    ...
    def _build_boundaries(
        ...
        for source_skill in range(len(policy.skills) - 1):
            ...
            next_entry = states[StateId(source_skill + 1, 0)]
            guard_entities = {
                name
                for name in next_entry.action_relevant_frames
                if self._is_scene_entity(name)
            }
            ...
            boundary_external_entities = tuple(
                name
                for name in policy.frame_names
                if name in guard_entities
                and self._is_scene_entity(name)
                and all(
                    states[StateId(source_skill, local_index)].demo_relation_priors[
                        name
                    ][mode, 1]
            ...
            sparse_library = set(
                self._factors_for_references(
                    data,
                    boundary_external_entities,
                    scene_entities=tuple(
                        name
            ...
            # A boundary may depend on physical task geometry that is no
            # longer an action stream (e.g. two linked objects at contact).
            # Reuse only factors already selected as progress evidence in the
            # terminal window, rather than admitting a second unscreened
            # boundary-only candidate library.
            boundary_relevance_start = max(
                0,
                start - self.config.scene_max_neighborhood,
            )
            progress_candidate_library = {
                factor_id
                for local_index in range(boundary_relevance_start, skill.duration)
                for factor_id in states[
                    StateId(source_skill, local_index)
                ].scene_factor_models
            }
            # Only a factor selected in the actual terminal window has already
            # passed a state-recognition screen relevant to this boundary.
            # Neighborhood-only progress factors remain candidates, but must
            # still prove that they discriminate terminal readiness here.
            progress_selected_library = {
                factor_id
                for local_index in range(start, skill.duration)
                for factor_id in states[
                    StateId(source_skill, local_index)
            ...
            sparse_library.update(progress_candidate_library)
            boundary_only_library = sparse_library.difference(
                progress_selected_library
            )
            scene_conditions = {}
            scene_condition_thresholds = {}
            for factor_id in sorted(sparse_library):
                try:
                ...
                scene_fitted = self._fit_boundary_scene_condition(
                    ...
                    policy,
                    factor_id,
                    require_discrimination=factor_id in boundary_only_library,
                )
    ...
    def build(
        ...
        *,
        ...
        scene_pose_names = set(demonstrations[0].scene_entity_poses)
        available_relation_frames = set(policy.frame_names).union(scene_pose_names)
        unknown_recoverable = recoverable.difference(available_relation_frames)
        if unknown_recoverable:
            raise ValueError(
                "可恢复关系实体不在动作参考或场景观测中："
                f"{sorted(unknown_recoverable)}"
            )
        relation_frames = tuple(policy.frame_names) + tuple(
            sorted(recoverable.difference(policy.frame_names))
        )
        aligned = self._align_demonstrations(policy, demonstrations)
        previous_relation_frames = getattr(self, "_state_relation_frames", None)
        self._state_relation_frames = relation_frames
        try:
            states, skill_states = self._build_states(policy, aligned)
        finally:
            if previous_relation_frames is None:
                del self._state_relation_frames
            else:
                self._state_relation_frames = previous_relation_frames
    ...
    def _assign_transaction_groups(
        self,
        left_model: TSFTaskModel,
        right_model: TSFTaskModel,
    ...
    def _complete_bimanual_boundary_scene_conditions(
        ...
        def add_dependency(
            ...
            terminal_node = source_model.state(
                source_boundary.local_completion_model.terminal_states[-1]
            )
            if any(
                peer_frame not in terminal_node.mode_action_relevant_frames[mode]
    ...
    @staticmethod
    def _independent_reference_event_frames(
        model: TSFTaskModel,
    ) -> set[str]:
        """Physical entities linked or pending on an independently controlled arm."""
    ...
    def _independent_reference_geometry_evidence(
        self,
        model: TSFTaskModel,
        aligned: dict[int, _AlignedSkillData],
        skill_index: int,
        ...
        """Measure a stable directed target relative to an independent frame."""
        ...
        local = np.stack(
            [
                _relative_batch(
                    data.frames[frame][demonstration],
    ...
    def _refine_independent_reference_action_relevance(
        self,
        model: TSFTaskModel,
        peer_model: TSFTaskModel,
        demonstrations: Sequence[DynaMACDemonstration],
    ) -> None:
        """Complete the shared action-relevance audit for independent frames.

        Marginal prediction alone cannot identify causality when another
        controller happens to move synchronously in every demonstration.  An
        independently actuated reference therefore keeps action authority only
        if the normal task also supplies a shared relation path or a stable
        directed relative-geometry target that is not already represented by
        the peer's relation to a selected entity. Task-parameter candidacy and all
        relation evidence remain untouched.
        """

        frame = f"{'right' if model.arm_id == 'left' else 'left'}_ee"
        aligned = self._align_demonstrations(model.base_policy, demonstrations)
        peer_event_frames = self._independent_reference_event_frames(peer_model)
        action_audit = {
            (int(record["skill_index"]), int(record["mode"])): record
            for record in model.builder_config["action_stream_relevance"]
        }
        rejected: set[tuple[int, int]] = set()

        for skill_index, skill in enumerate(model.base_policy.skills):
            for mode in range(len(skill.mode_priors)):
                first = model.state(model.skill_states[skill_index][0])
                if frame not in first.mode_selected_frames[mode]:
                    continue
                record = action_audit[(skill_index, mode)]
                metrics = record["frame_metrics"][frame]
                marginally_relevant = bool(metrics["retained"])
                physical_frames = tuple(
                    candidate
                    for candidate in first.mode_selected_frames[mode]
                    if self._is_scene_entity(candidate)
                )
                relation_path = tuple(
                    sorted(
                        self._linked_frames_in_skill_mode(
                            model, skill_index, mode
                        )
                        & peer_event_frames
                    )
                )
                represented_peer_entities = tuple(
                    sorted(set(physical_frames) & peer_event_frames)
                )
                geometry = self._independent_reference_geometry_evidence(
                    model,
                    aligned,
                    skill_index,
                    mode,
                ...
                alternative_frames = tuple(
                    candidate
                    for candidate in first.mode_action_relevant_frames[mode]
                    if candidate != frame
                )
                alternative_support_complete = all(
                    any(
                        skill.streams[candidate].is_active(
                            mode, state_id.local_index
                        )
                        for candidate in alternative_frames
                    )
                    for state_id in model.skill_states[skill_index]
                )
                if relation_path:
                    ...
                    physical_frames
                    and not represented_peer_entities
                    and geometry["eligible"]
                ):
                    structurally_identified = True
                    reason = "unrepresented_shared_configuration"
                else:
                    structurally_identified = False
                    reason = "unidentified_independent_reference_dependency"

                metrics["independent_reference_identifiability"] = {
                    "eligible": structurally_identified,
                    "reason": reason,
                    "physical_frames": list(physical_frames),
                    "relation_path_frames": list(relation_path),
                    "represented_peer_entities": list(represented_peer_entities),
                    "geometry": geometry,
                    "alternative_support_complete": alternative_support_complete,
                }
                if marginally_relevant and not structurally_identified:
                    if not alternative_support_complete:
                        metrics["reason"] = (
                            "required_for_poe_coverage_despite_unidentified_dependency"
                        )
                        continue
                    metrics["marginal_reason"] = metrics["reason"]
                    metrics["reason"] = reason
                    metrics["retained"] = False
                    record["retained_frames"] = [
                        candidate
                        for candidate in record["retained_frames"]
                        if candidate != frame
                    ]
                    rejected.add((skill_index, mode))

        for skill_index, mode in sorted(rejected):
            for state_id in model.skill_states[skill_index]:
                node = model.states[state_id]
                modes = list(node.mode_action_relevant_frames)
                modes[mode] = tuple(candidate for candidate in modes[mode] if candidate != frame)
                if not any(
                    model.base_policy.skills[skill_index].streams[candidate].is_active(
                        mode, state_id.local_index
                    )
                    for candidate in modes[mode]
                ):
                    raise ValueError(
                        "独立参考系不可辨识且移除后动作支持为空："
                        f"{model.arm_id}/skill={skill_index}/mode={mode}/state={state_id}"
                    )
                node.mode_action_relevant_frames = tuple(modes)
                node.action_relevant_frames = tuple(
                    candidate
                    for candidate in node.selected_frames
                    if any(
                        candidate in relevant
                        for relevant in node.mode_action_relevant_frames
                    )
                )
    ...
    def build_bimanual(
        ...
        *,
        recoverable_frames: Sequence[str] | None = None,
    ) -> tuple[TSFTaskModel, TSFTaskModel]:
        ...
        self._refine_independent_reference_action_relevance(
            left_model,
            right_model,
            paired_left,
        )
...
__all__ = ["TSFTaskModelBuilder", "TSFTaskModelConfig"]
