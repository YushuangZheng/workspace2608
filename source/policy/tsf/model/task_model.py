"""Unified offline task model for Task-State Feedback execution."""
...
class StateNode:
    ...
    # The base policy's task-parameter selector defines the complete candidate
    # bank for relation/event
    # semantics.  The offline harmful-stream screen removes only candidates
    # with repeatable negative held-out action value; its retained mask is the
    # robot-motion support shared by normal PoE, progress and local completion.
    action_relevant_frames: tuple[str, ...]
    mode_action_relevant_frames: tuple[tuple[str, ...], ...]
    ...
    def __post_init__(self) -> None:
        ...
        for mode, relevant in enumerate(self.mode_action_relevant_frames):
            if not set(relevant).issubset(self.mode_selected_frames[mode]):
                raise ValueError("动作相关流必须是基础策略模态候选流的子集")
...
@dataclass
class TSFTaskModel:
    """TSF task-state statistics bound to one fitted DynaMAC policy."""
    ...
    def __post_init__(self) -> None:
        if not self.base_policy.fitted:
            raise RuntimeError("TSFTaskModel 需要已拟合的基础 DynaMAC")
        policy_digest = self.base_policy.identity_digest()
        if not self.base_policy_digest:
            self.base_policy_digest = policy_digest
        elif self.base_policy_digest != policy_digest:
            raise ValueError("TSF 任务模型与基础 DynaMAC 内容摘要不匹配")
        if set(self.states) != {
        ...
        if len(self.states) != expected:
            raise ValueError("TSF 任务状态数量与基础 DynaMAC 不一致")
    ...
    def query_state(
        ...
        *,
        ...
        if state_id not in self.states:
            raise KeyError(f"TSF 任务模型不存在状态 {state_id}")
    ...
    def state(self, state_id: StateId) -> StateNode:
        try:
            ...
            raise KeyError(f"TSF 任务模型不存在状态 {state_id}") from exc
        ...
        for event_id, candidate in self.link_pending_events.items():
            if enabled(event_id):
                ...
                if candidate.release_state is not None:
                    transitions.append(
                        (
                            global_index[candidate.release_state],
                            1,
                            event_id.token + ":release",
                            None,
                        )
                    )
    ...
    def save(self, path: str | Path) -> None:
            ...
            state_records.append(
                {
                    ...
                    # The TSF sidecar may remove a base-policy-selected peer
            ...
            link_pending_records.append(
                {
                    ...
                    "release_state": (
                        None
                        if candidate.release_state is None
                        else self._state_meta(candidate.release_state)
                    ),
                    "local_means": f"{prefix}__means",
                    "local_covariances": f"{prefix}__covariances",
                    "gripper_commands": f"{prefix}__gripper",
        ...
        metadata = {
            "schema": "essay2608.tsf.task_model.v1",
            "schema_version": self.schema_version,
            "arm_id": self.arm_id,
            "base_policy_digest": self.base_policy_digest,
            "base_policy_selection_semantics_id": self.base_policy.selection_semantics_id,
    ...
    @classmethod
    def load(cls, path: str | Path, base_policy: DynaMAC) -> TSFTaskModel:
        path = Path(path)
        with np.load(path, allow_pickle=False) as archive:
            metadata = json.loads(str(archive["metadata_json"].item()))
            schema = metadata.get("schema")
            if schema not in {
                "essay2608.tsf.task_model.v1",
                "essay2608.closed_loop_task_model.v7",
            }:
                raise ValueError("不支持的 TSF 任务模型 schema")
            stored_digest = metadata.get(
                "base_policy_digest", metadata.get("base_policy_fingerprint")
            )
            if stored_digest != base_policy.identity_digest():
                raise ValueError("TSF 任务模型与提供的基础 DynaMAC 内容摘要不匹配")
            if (
                metadata.get("base_policy_selection_semantics_id")
                != base_policy.selection_semantics_id
            ):
                raise ValueError("TSF 任务模型与基础 DynaMAC 选择语义不匹配")
            ...
            for record in metadata["link_pending_events"]:
                ...
                link_pending_events[event_id] = LinkPendingCandidate(
                    ...
                    release_state=(
                        None
                        if record.get("release_state") is None
                        else StateId.from_tuple(record["release_state"])
                    ),
                    support_fraction=float(record["support_fraction"]),
                    demonstration_indices=tuple(record["demonstration_indices"]),
                    event_local_indices=tuple(record["event_local_indices"]),
        ...
        return cls(
            ...
            base_policy_digest=str(stored_digest),
            schema_version=int(metadata["schema_version"]),
        )


__all__ = ["TSFTaskModel", "StateNode"]
