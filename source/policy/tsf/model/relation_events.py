...
class LinkRecoveryAnchor:
    ...
    # relation ownership is stored separately in TSFTaskModel.link_origins.
...
class LinkPendingCandidate:
    ...
    local_covariances: Array
    gripper_commands: Array
    # First cross-demonstration causal opening after the unresolved close.
    # The soft LINK hypothesis is active before, but not at, this state.  None
    # means a later explicit relation event owns the end of the interval.
    release_state: StateId | None = None
    support_fraction: float = 1.0
    demonstration_indices: tuple[int, ...] = ()
    event_local_indices: tuple[int, ...] = ()
    ...
    def __post_init__(self) -> None:
        ...
        if self.release_state is not None and self.release_state <= self.candidate_state:
            raise ValueError("LINK_PENDING 释放状态必须晚于候选闭爪状态")
        means = np.asarray(self.local_means, dtype=np.float64)
        covariance = np.asarray(self.local_covariances, dtype=np.float64)
        gripper = np.asarray(self.gripper_commands, dtype=np.float64)
