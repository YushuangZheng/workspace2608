...
from ..inference.belief_updater import TSFBelief
from ..model.relation_events import RelationEventId, RelationStateKey
...
class RelationVerificationRequest:
    ...
    committed_occurrence: bool = False

    def __post_init__(self) -> None:
        if not self.arm_id or not self.frame_id:
            raise ValueError("主动关系验证请求需要非空机械臂和参考系")
        if self.relation != "linked":
            raise ValueError("流角色路由器只为 linked 关系生成主动验证请求")
        if (
...
class FrameRoleSnapshot:
    ...
    unresolved_pending_link_events: tuple[RelationEventId, ...] = ()
...
class FrameRoleRouter:
    ...
    def __init__(
        ...
        # Several entities can exhibit the same LINK transition because one
        # is carried through another (for example, payload-on-support).  Such
        # co-motion is still useful task-state evidence, but only an entity
        # that already had action authority can ground an executable LINK
        # repair.  Derive this distinction exclusively from the existing
        # event timing and offline action-relevance model; no force/contact
        # observation or additional online state is introduced.
        self._indirect_link_events = self._derive_indirect_link_events()
        ...
        # A LINK_PENDING occurrence becomes actionable only after its learned
        # relation-changing edge has actually committed.  Keep that causal
        # identity separate from merely visiting a state in the occurrence's
        # offline context; otherwise a later repeated event can disappear when
        # its object frame is not an active DynaMAC action stream.
        self._unresolved_pending_link_events: set[RelationEventId] = set()
        self._last_committed_state: StateId | None = None

    def _link_events_overlap(
        self,
        left: RelationEventId,
        right: RelationEventId,
    ) -> bool:
        """Return whether two LINKs are one demonstrated co-motion event."""

        if (
            left == right
            or left.transition != "link"
            or right.transition != "link"
            or left.skill_index != right.skill_index
            or left.mode != right.mode
        ):
            return False
        lhs = self.task_model.link_anchors[left]
        rhs = self.task_model.link_anchors[right]
        tolerance = int(
            self.task_model.builder_config.get(
                "relation_event_alignment_tolerance", 0
            )
        )
        timing_close = bool(
            len(lhs.event_local_indices) == len(rhs.event_local_indices)
            and all(
                abs(int(a) - int(b)) <= tolerance
                for a, b in zip(
                    lhs.event_local_indices,
                    rhs.event_local_indices,
                    strict=True,
                )
            )
        )
        return timing_close and not set(lhs.linked_entry_states).isdisjoint(
            rhs.linked_entry_states
        )

    def _derive_indirect_link_events(self) -> frozenset[RelationEventId]:
        """Identify co-moving LINK evidence without independent action authority.

        A multi-frame event is changed only when exactly one participating
        frame occurs in the already learned pre-event action-relevance mask.
        Zero or multiple candidates are ambiguous and retain the original
        behavior rather than guessing a recovery target.
        """

        remaining = set(self.task_model.link_anchors)
        groups: list[set[RelationEventId]] = []
        while remaining:
            seed = remaining.pop()
            component = {seed}
            frontier = [seed]
            while frontier:
                ...
                neighbors = {
                    ...
                    if self._link_events_overlap(current, candidate)
                }
        ...
        indirect: set[RelationEventId] = set()
        for group in groups:
            earliest = min(
                min(self.task_model.link_anchors[event].linked_entry_states)
                for event in group
            )
            pre_event_states = tuple(state for state in self._states if state < earliest)
            actionable = set()
            for event in group:
                for state in pre_event_states:
                    node = self.task_model.state(state)
                    # Motion-mode indices are local to each skill.  The LINK
                    # event's mode must not be projected backward onto an
                    # earlier skill, so consult that state's already-frozen
                    # union of mode-specific action authority.
                    if event.frame_id in node.action_relevant_frames:
                        actionable.add(event)
                        break
            if len(actionable) == 1:
                indirect.update(group.difference(actionable))
    ...
    def reset(self) -> None:
        ...
        self._unresolved_pending_link_events.clear()
        self._last_committed_state = None

    def active_direct_link_event(
        self,
        frame_id: str,
        state_id: StateId,
        *,
        mode_by_skill: Mapping[int, int],
    ) -> RelationEventId | None:
        """Return the unique direct LINK still active at ``state_id``.

        The online relation posterior decides whether a relation is physically
        present.  This lookup supplies only its learned causal identity: the
        latest selected-mode LINK not superseded by a selected-mode UNLINK.
        Co-motion events without independent action authority are excluded so
        an indirectly carried entity cannot become a coordination target.
        """

        if state_id not in self.task_model.states:
            raise KeyError(f"流角色查询未知状态 {state_id}")
        candidates: list[tuple[StateId, RelationEventId]] = []
        for event, anchor in self.task_model.link_anchors.items():
            if event.frame_id != frame_id:
                continue
            if mode_by_skill.get(event.skill_index) != event.mode:
                continue
            first_linked = min(anchor.linked_entry_states)
            if first_linked > state_id:
                continue
            invalidated = any(
                unlink.frame_id == frame_id
                and mode_by_skill.get(unlink.skill_index) == unlink.mode
                and first_linked < metadata.release_state <= state_id
                for unlink, metadata in self.task_model.unlink_events.items()
            )
            if not invalidated:
                candidates.append((first_linked, event))
        if not candidates:
            return None
        latest = max(state for state, _event in candidates)
        active = [event for state, event in candidates if state == latest]
        if len(active) != 1 or active[0] in self._indirect_link_events:
            return None
        return active[0]

    def selected_mode_unlink_event_in_skill(
        self,
        frame_id: str,
        skill_index: int,
        *,
        ...
        """Return a learned direct UNLINK for ``frame_id`` in one skill.

        This is a causal-topology query, not an online relation decision.  It
        is used when another arm is acquiring the same frame: a selected-mode
        UNLINK by the currently linked arm in that acquisition skill denotes
        sequential ownership transfer, rather than sustained shared support.
        """

        selected_mode = mode_by_skill.get(skill_index)
        if selected_mode is None:
            return None
        candidates = sorted(
            event
            for event in self.task_model.unlink_events
            if event.frame_id == frame_id
            and event.skill_index == skill_index
            and event.mode == selected_mode
        )
    ...
    def commit(
        ...
        *,
        ...
        self._unresolved_pending_link_events = set(
            snapshot.unresolved_pending_link_events
        )
        ...
        if belief is not None:
            ...
            for frame, estimate in belief.relation_estimates.items():
                ...
                if estimate.decision_state == RelationDecision.EXTERNAL:
                    # Cache trusted action authority, not the posterior
                    # confidence already consumed by categorical role routing.
                    self._last_trusted_weights[frame] = reliability
    ...
    def commit_boundary_entry(
        ...
        *,
        ...
        lifecycle_snapshot = replace(
            ...
            unresolved_pending_link_events=tuple(
                sorted(self._unresolved_pending_link_events)
            ),
        )
        self.commit(
            lifecycle_snapshot,
        ...
        entry_mode = self._mode_for_state(entry_state, mode_by_skill)
        source_mode = self._mode_for_state(source_snapshot.state_id, mode_by_skill)
        if entry_mode is None:
            entry_mode = int(
                self.task_model.base_policy.selected_mode_path[
                    entry_state.skill_index
                ]
            )
        if source_mode is None:
            source_mode = int(
                self.task_model.base_policy.selected_mode_path[
                    source_snapshot.state_id.skill_index
                ]
            )
        source_commands = self.task_model.state(
            source_snapshot.state_id
        ).gripper_commands
        entry_commands = self.task_model.state(entry_state).gripper_commands
        if (
            0 <= source_mode < len(source_commands)
            and 0 <= entry_mode < len(entry_commands)
        ):
            source_gripper = np.asarray(source_commands[source_mode], dtype=np.float64)
            entry_gripper = np.asarray(entry_commands[entry_mode], dtype=np.float64)
            closes_relation = bool(
                source_gripper.shape == entry_gripper.shape
                and np.all(source_gripper > 0.5)
                and np.all(entry_gripper <= 0.5)
            )
            if closes_relation:
                self._unresolved_pending_link_events.update(
                    event_id
                    for event_id, candidate in self.task_model.link_pending_events.items()
                    if event_id.mode == entry_mode
                    and candidate.candidate_state == entry_state
                )
    ...
    def _candidate_frames(
        ...
        if mode is None:
            return tuple(dict.fromkeys((*node.selected_frames, *node.action_relevant_frames)))
        if mode < 0 or mode >= len(node.mode_selected_frames):
            raise IndexError("mode_index 超出流角色模型的模态范围")
        return tuple(
            dict.fromkeys(
                (
                    *node.mode_selected_frames[mode],
                    *node.mode_action_relevant_frames[mode],
                )
            )
        )
    ...
    def _is_formal_unlink_confirmation_state(
        self,
        frame: str,
        state_id: StateId,
        mode_by_skill: Mapping[int, int] | None,
    ) -> bool:
        """Return whether this state belongs to causal UNLINK confirmation.

        The observation at the release state is still the response to the
        preceding command.  It can therefore retain a valid ``linked``
        posterior even though the opening command and the following normal
        separation motion are what make detachment observable.  Treating that
        causal lag as a failed UNLINK suppresses the very motion needed to
        update the relation filter.

        ``legal_reentry_states`` is the finite, cross-demonstration-supported
        external interval learned for this UNLINK event.  Reuse that same
        physical interval for natural confirmation instead of imposing an
        unrelated one-successor runtime window.  Once execution leaves it,
        persistent linkage follows the ordinary recovery path.
        """
        ...
        for candidate_mode in modes:
            ...
            if not any(
                metadata.release_state == state_id
                or state_id in metadata.legal_reentry_states
    ...
    def _committed_pending_resolution_due(
        self,
        event_id: RelationEventId,
        context_state: StateId,
    ) -> bool:
        """Whether an executed Pending occurrence must be resolved now.

        A committed close makes the exact occurrence persistent, but does not
        imply that every subsequent cycle needs an auxiliary probe.  Normal
        task motion may supply the missing co-motion evidence first.  The
        relation becomes causally mandatory immediately before its learned
        UNLINK/release action: allowing that action while the LINK is still
        unresolved would erase the physical question and silently skip the
        same event.  This rule is occurrence-independent and follows only the
        learned relation topology.
        """

        candidate = self.task_model.link_pending_events[event_id]
        release = candidate.release_state
        if release is None:
            return False
        if context_state == release:
            return True
        return release in self.task_model.state(context_state).topology.successors
    ...
    def _verification_requests(
        ...
        *,
        ...
        requests: dict[str, RelationVerificationRequest] = {}
        context_state = max(
        ...
        for event_id, candidate in sorted(self.task_model.link_pending_events.items()):
            ...
            features = belief.runtime_features
            committed_occurrence = event_id in self._unresolved_pending_link_events
            committed_resolution_due = bool(
                committed_occurrence
                and self._committed_pending_resolution_due(event_id, context_state)
            )
            if (
                estimate is None
                ...
                < self.config.minimum_tracking_reliability
                or (
                    not committed_resolution_due
                    and (
                        not self._pending_relation_needed(event_id, mode_by_skill)
            ...
            request = RelationVerificationRequest(
                ...
                committed_occurrence=committed_resolution_due,
            )
            requests[request.event_id.token] = request
        ...
        for event_id in sorted(self._unresolved_formal_link_events):
            if (
                ...
                or event_id in self._indirect_link_events
            ):
                continue
            anchor = self.task_model.link_anchors.get(event_id)
            if anchor is None or not anchor.linked_entry_states:
    ...
    def route(
        ...
        *,
        ...
        # even when it is not a base-policy-selected action stream; otherwise the causal
        ...
        unresolved_pending_link_events = set(self._unresolved_pending_link_events)
        naturally_confirming_events: set[RelationEventId] = set()
        features = belief.runtime_features
        context_state = max(
            (state_id, belief.progress.estimated_state),
            key=self._global_index.__getitem__,
        )
        for event_id in tuple(unresolved_pending_link_events):
            candidate = self.task_model.link_pending_events.get(event_id)
            estimate = belief.relation_estimates.get(event_id.frame_id)
            active = (
                None
                if candidate is None
                else self.task_model.active_link_pending_candidate(
                    candidate.frame_id,
                    context_state,
                    mode_by_skill,
                )
            ...
            if (
                candidate is None
                or active is None
                or active.event_id != event_id
                or (
                    estimate is not None
                    and estimate.decision_state == RelationDecision.LINKED
                )
            ):
                unresolved_pending_link_events.discard(event_id)
                continue

            # LINK_PENDING is created only for the physical entity directly
            # approached by the learned close.  Once that exact close has
            # committed, normal task motion is the preferred relation probe.
            # A stable posterior and fresh action-conditioned evidence that
            # both support EXTERNAL therefore mean that this occurrence did
            # not establish its intended LINK.  Route the existing LINK
            # repair intent immediately; the shared mismatch tracker still
            # requires its ordinary multi-cycle persistence, so one transient
            # response cannot trigger recovery.  Low-information evidence is
            # left to VERIFY_LINK, while LINKED was consumed above as natural
            # confirmation.
            if (
                estimate is not None
        ...
        for frame in (*candidate_frames, *confirmed_monitors):
            ...
            formal_unlink_confirmation = self._is_formal_unlink_confirmation_state(
                frame,
                state_id,
                mode_by_skill,
            )
            unresolved_low_information_link = bool(
                ...
                < self.config.minimum_information_weight
                and any(
                    event_id in self._unresolved_formal_link_events
                    for event_id in current_link_events
                )
                and features.frame_pair_available.get(frame, False)
                and features.paired_tracking_reliability.get(frame, 0.0)
                >= self.config.minimum_tracking_reliability
            )
            compatibility = (
                0.5
            ...
            if confirmation_events:
                ...
                formal_unlink_confirmation
                and expected == RelationDecision.EXTERNAL
                and actual in {RelationDecision.UNKNOWN, RelationDecision.LINKED}
            ):
                # The opening command has now been committed.  Its object
                # frame is therefore the ordinary external action reference
                # for the learned separation motion, even though the relation
                # posterior still describes the response to the preceding
                # command.  Executing that already-selected DynaMAC stream is
                # what creates the first observable UNLINK evidence.  This
                # does not confirm detachment: persistent reliable linkage in
                # the following observation still enters RECOVER.
                role = (
                    FrameRole.EXECUTE if selected_offline else FrameRole.DEFER
                )
                # The learned interval already authorizes this stream while
                # the causal UNLINK response is observed.  Applying the
                # expected-relation probability again as a precision scale
                # would deform an otherwise valid frozen-expert action.
                weight = reliability if selected_offline else 0.0
                monitor = True
                blocks = False
                intent = None
            elif unresolved_low_information_link:
                # The categorical EXTERNAL value can be persistence carried
                # from before the formal LINK event.  With no informative
                # post-event response it is not a failed grasp and must not
                # compete with the VERIFY_LINK request generated below.
                # Keep the physical frame out of the action PoE and let the
                # existing bounded verifier resolve the single uncertainty.
                role = FrameRole.DEFER
                weight = 0.0
                monitor = True
                blocks = False
                ...
                # A candidate retained only for relation/event semantics has
                # no robot-motion authority.  Its unexcited EXTERNAL relation
                # cannot freeze the driving streams needed to create physical
                # evidence; a later reliable LINK decision still enters the
                # ordinary mismatch/recovery branch below.  Event-supported
                # LINK monitors are deliberately unaffected by this rule.
                ...
                role = FrameRole.EXECUTE if selected_offline else FrameRole.DEFER
                # EXTERNAL has already been resolved categorically here.
                # Preserve the selected expert's original relative precision;
                # q(external) remains a soft weight only for the initial
                # UNKNOWN bootstrap that creates identifying motion.
                weight = reliability if selected_offline else 0.0
                monitor = False
                blocks = False
                intent = None
                ...
                expected == RelationDecision.LINKED
                and actual == RelationDecision.EXTERNAL
                and current_link_events
                and all(
                    event_id in self._indirect_link_events
                    for event_id in current_link_events
                )
            ):
                # The relation remains visible to progress inference, but its
                # loss cannot be repaired by directly opening and re-grasping
                # an entity that never had action authority.  The causally
                # operable member of the same LINK group retains the ordinary
                # RECOVER path.  RLBench task success still rejects a rollout
                # if the indirectly carried entity is not physically restored.
                role = FrameRole.DEFER
                weight = 0.0
                monitor = True
                blocks = False
        ...
        # A directly selected frame can expose the same mismatch through the
        # ordinary expected-vs-actual role table and through the exact
        # committed Pending occurrence above.  Keep one arm-frame intent and
        # preserve the occurrence-bearing version so recovery uses the
        # matching learned anchor rather than guessing from the current state.
        unique_recovery_intents: dict[tuple[str, str], RelationRecoveryIntent] = {}
        for intent in recovery_intents:
            key = (intent.arm_id, intent.frame_id)
            previous = unique_recovery_intents.get(key)
            if previous is None or (
                previous.origin_event_id is None
                and intent.origin_event_id is not None
            ):
                unique_recovery_intents[key] = intent
        snapshot = FrameRoleSnapshot(
            state_id=state_id,
            decisions=decisions,
            recovery_intents=tuple(
                unique_recovery_intents[key]
                for key in sorted(unique_recovery_intents)
            ),
            ...
            unresolved_pending_link_events=tuple(
                sorted(unresolved_pending_link_events)
            ),
        )
        if commit:
    ...
    def route_fixed(
        ...
        *,
        ...
        Relation posteriors remain available to progress inference
        ...
        # task-parameter candidate mask; offline relevance is part of the
        # proposed dynamic stream router, not a hidden baseline change.
        node = self.task_model.state(state_id)
        mode = self._mode_for_state(state_id, mode_by_skill)
        selected_frames = (
            node.selected_frames
            if mode is None
            else node.mode_selected_frames[mode]
        )
        decisions = {}
