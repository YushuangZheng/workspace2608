"""Object-centric skill-entry guard evaluation."""
...
class EntryGuard:
    ...
    def __init__(
        ...
        *,
        ...
        self.robot_runtime_variance = float(robot_runtime_variance)
        if (
            not math.isfinite(self.robot_runtime_variance)
            or self.robot_runtime_variance < 0.0
        ):
            raise ValueError("边界机器人运行时方差必须为有限非负数")
        self._robot_ee_frames = frozenset(f"{arm}_ee" for arm in task_models)
        self.reset()

    def _scene_runtime_covariance(
        self,
        factor: FactorId,
        dimension: int,
    ) -> np.ndarray | None:
        endpoints = {factor.source}
        if factor.target is not None:
            endpoints.add(factor.target)
        robot_terms = len(endpoints.intersection(self._robot_ee_frames))
        return robot_pose_observation_covariance(
            self.robot_runtime_variance,
            robot_terms,
            dimension=dimension,
        )
    ...
    def _transition_preparation(
        ...
        *,
        ...
        if not events:
            return None
        establishable_conditions = {
            f"{boundary.boundary_id.arm_id}/{event_id.frame_id}"
            for event_id in events
        }
        unmet = [result for result in guard_results if not result.satisfied]
        # A transition-preparation close may establish only its own learned
        # LINK condition.  Every independent relation and scene condition must
        # already be ready, and no IK/executor result can waive them.
        if any(
            result.condition_id.kind != ConditionKind.GUARD_RELATION
            or result.condition_id.token not in establishable_conditions
            for result in unmet
        ):
            return None
    ...
    def evaluate(
        ...
        *,
        ...
        for key, condition in sorted(own_relation_conditions):
            ...
            condition_id = ConditionId(
                ConditionKind.GUARD_RELATION, relation_arm, key
            )
        ...
        # beta_t has already incorporated robot, relation, and sparse-scene
        # evidence.  Local completion therefore reads terminal posterior mass
        # exactly once; boundary-specific relations and scene factors remain
        # explicit guards below instead of being multiplied into a second
        # task-state score.
        evidence_available = progress_supported
        local_score = end_probability
        local_raw = bool(
        ...
        for factor, distribution in sorted(scene_conditions):
            ...
            observation_covariance = self._scene_runtime_covariance(
                factor,
                int(distribution.covariance.shape[0]),
            )
            compatibility = (
                0.0
                if value is None
                else distribution.compatibility(value, observation_covariance)
            )
        ...
        if preparation is not None:
            ...
            # commit. Surface its exact Pending occurrence to recovery even
