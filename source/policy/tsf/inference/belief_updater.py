"""Single-pass coupled relation and progress belief update."""
...
class BeliefUpdaterConfig:
    ...
    def from_mapping(cls, value: Mapping[str, Any]) -> BeliefUpdaterConfig:
        ...
        if unknown:
            raise ValueError(f"信念更新配置包含未知分区：{sorted(unknown)}")
        arguments = {}
        for name, section_type in sections:
            raw = value.get(name, {})
            if not isinstance(raw, Mapping):
                raise TypeError(f"信念更新配置分区 {name} 必须为对象")
    ...
    def from_json(cls, path: str | Path) -> BeliefUpdaterConfig:
        ...
        if not isinstance(value, Mapping):
            raise TypeError("信念更新配置文件根节点必须为对象")
    ...
    def update_frozen(
        ...
        *,
        ...
        if self._last_tick is not None and observation.tick <= self._last_tick:
            raise ValueError("BeliefUpdater 每个递增控制周期只能更新一次")
        features = self.feature_builder.build(observation, self._previous_observation)
        unknown_uninformative = set(uninformative_relation_frames).difference(
            self.task_model.relation_frames
        )
        if unknown_uninformative:
            raise KeyError(
                "关系信息屏蔽引用未知参考系："
                f"{sorted(unknown_uninformative)}"
            )
        if uninformative_relation_frames:
            # A controller action computed directly from a frame's observed
            # motion cannot provide independent evidence that the commanded
            # arm is physically linked to that same frame.  Keep the real
            # kinematics in RuntimeFeatures for diagnostics and state scoring,
            # but give that causally confounded relation update zero evidence
            # weight.  The persistent binary posterior is therefore preserved
            # until a genuinely identifying action/response is observed.
