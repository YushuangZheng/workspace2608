...
class ReentrySelector:
    ...
    def select_forced_same_state(
        ...
        *,
        ...
        """Resume the frozen pre-recovery state without legal-state search.

        This operation exists only for the controlled ``retry_same_state``
        ablation.  It still records the complete-state score for audit, but it
        deliberately does not use that score to choose or reject another
        state.  Relation repair has already completed before this method can
        be called, and no skill boundary is crossed by construction.
        """

        if state_id not in self.task_model.states:
            raise KeyError(f"原状态重入包含未知状态：{state_id}")
        ...
        return ReentryEvaluation(
            decision=ReentryDecision(state_id, score, None),
            scores=scores,
            rejection_reasons={},
