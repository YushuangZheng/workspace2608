...
- 提交 `e0668ac` 后冻结在线实现；[正式协议 v1](bimanual_relation_protocol.md)预注册 seed 8400–8409、七条件 70 次隔离试验、逐边/延迟/安全门和任务成功门；正式结果尚未运行。
- 正式 v1 的 70/70 文件完整、70/70 关系序列精确、应成功任务 40/40；但延长双持只有 7/10 在完整两秒维持物理 `both`，故整体未通过；[v1 报告](bimanual_relation_report_v1.md)保留三条失败的逐步归因和 v2 允许修改边界。
