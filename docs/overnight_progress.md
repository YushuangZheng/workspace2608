...
- 十条开发回放的四值准确率均值为 94.33%，左/右 micro F1 为 97.82%/95.41%，左/右假阳性为 0/2；
- 真值 `both` 为 901 步而推断仅 568 步，当前主要问题是接收边建立延迟/漏检，不是虚构关系；
- 五类反事实单元测试验证两条边不会因另一臂空抓、释放、丢失、长时双持或暂停而被 phase 计时器错误改写；
- [双臂在线关系说明](bimanual_relation_estimator.md)记录完整输入边界、指标和下一阶段门槛；当前不进入完整双臂策略训练。
