...
## 2026-08-04：双臂关系门控恢复 Phase 6 收尾

- 全部阶段成果经 [PR #4](https://github.com/YushuangZheng/workspace2608/pull/4) 合并到 `master`，合并提交为 `c28b18a`；
- 仓库分支收敛为唯一的 `master`，后续只在 `master` 上开发，已合并旧分支不再保留；
- 新增独立恢复监督层，覆盖正常、接收空抓、短暂丢失、共同持物期强丢失和发送臂释放后丢失；
- 四种对照严格区分原时钟专家、只关系门控、在线关系恢复和只替换当前关系真值的 Oracle 恢复；
- 最终冻结开发批次为 `30/30` 硬审计通过，动作目标跳变不超过 0.05 m，在线与 Oracle 均为 `15/15` 任务成功；
- 新增逐步原始证据、纯评分函数和硬审计，独立重算任务、关系、恢复时间、重抓、giver 保持、动作跳变和末端速度；
- 实现提交 `4214730`，正式协议提交 `e171cbe`，标签为 `bimanual-recovery-protocol-v1`；
- 正式 v1 按用户收尾指令在 95/200 条时中止，没有 summary、没有正式硬审计，明确不得声称正式通过或拼接补跑；
- 后续正式验收必须另建 v2 协议、全新 seed 和输出目录；完整双臂学习策略仍未实现。

## 2026-08-04：双臂在线关系估计 Phase 5 v2 正式结果

- 唯一一次正式运行完成七条件 × 十个未见 seed 的 70 个 worker，controller 退出码为 0；
- 70 个 JSON/NPZ 全部唯一成对落盘，物理条件、预注册推断序列和逐条关系硬门均为 `70/70`；
- v1 唯一失败的长双持由 `7/10` 改善为独立 v2 cohort 的 `10/10`，且十条任务全部成功；
- 四个应成功条件共 `40/40`，三个破坏持有关系的条件任务失败继续按预期因果后果记录；
- 加权四值准确率 98.46%，左右 micro-F1 为 98.56%/99.42%；
- 正式 summary 哈希为 `f7f6da53...b4d98`，只读硬审计返回 `all_trials_passed=true`；
- Phase 5 检测门已通过，但报告明确不将其等同于双臂恢复或完整学习策略完成。

## 2026-08-04：双臂在线关系估计 Phase 5 v2 修复与预注册

- v1 的三条长双持失败被定位为固定双臂笛卡尔目标造成的内部力与载荷沉降，不是估计器漏检；
- v2 只把长双持改为左右目标逐步跟随各自实测末端，保持双夹爪闭合与 phase clock 冻结；
- 新开发 seed `8303–8307` 为 `5/5` 物理长双持成立、`5/5` 任务成功，关系序列全部精确；
- 开发平均四值准确率 98.33%，左右平均 F1 为 98.69%/99.52%，summary 哈希为 `471ed34c...d946`；
- 实现提交 `914bdca` 固定源码指纹 `98dba7ce...ceb0c7`，估计配置和全部 v1 阈值保持不变；
- v2 预注册全新 seed `8500–8509` 和完整七条件 70 条矩阵，不与 v1 拼接，也不允许重试或替换；
- 正式审计通过前继续禁止完整双臂策略训练。
...
- 初始十条回放四值准确率为 94.33%、左右 F1 为 97.82%/95.41%；在线开发暴露接收边建立过慢和载荷转移伪断裂；
- 以标定 q01–q99 建立占用平台，并要求瞬时断裂与滑窗漂移共同触发运动学解除；最终离线回放为 98.30%，左右 F1 为 98.54%/99.44%；
- 五类纯状态机反例覆盖接收空抓/延迟、发送提前释放、接收后丢失、延长双持和单臂暂停；
- 标定配置支持验证后反序列化，并拒绝只设置建立或解除距离阈值的非法组合；
- 新增隔离在线评测器，保存逐步输入、物理真值、推断分数、基础/干预动作和事件；最终开发 seed 8302 的七类干预均物理成立且推断序列逐段正确；
- 新增 `docs/bimanual_relation_estimator.md`，记录输入边界、数据划分、完整指标、证据哈希和进入策略训练前的在线门槛。

## 2026-08-04：双臂在线关系估计 Phase 5 正式预注册

- 实现提交 `e0668ac` 固定在线源码指纹 `95d6e556...b1f23b` 和配置哈希 `9878468c...68ade`；
- 预注册全新 seed `8400–8409`、七种条件和 70 个精确 worker，不允许失败重试或替换；
- 每条同时约束物理干预成立、完整关系序列、四值准确率、左右 F1、建立/解除延迟及条件特有安全门；
- 正常、延迟、长双持和暂停各要求至少 `9/10` 任务成功，三个破坏持有关系的条件不把预期任务失败混为检测失败；
- 新增只读硬审计，逐步重算物理/推断标签、指标、干预语义、实验指纹和精确磁盘成员；
- 正式结果通过前继续禁止完整双臂策略训练。
- 首次正式审计暴露 JSON 的 Python double 控制周期与 NPZ float32 控制周期造成约 `5e-8 s` 延迟重算差；仅将等值比较容差改为 `1e-7`，不改变任何协议门或正式结果。

## 2026-08-04：双臂在线关系估计 Phase 5 正式 v1 结果

- 唯一一次正式运行完成七条件 × 十 seed 的 70 个 worker，70 个 JSON/NPZ 全部唯一落盘；
- 七类物理/推断压缩序列均为 `10/10` 精确，四个应成功条件共 `40/40` 任务成功；
- 加权四值准确率 98.40%，左右合并 F1 为 98.45%/99.43%，空抓右边 0 假阳性；
- `prolonged_both_hold` 只有 `7/10` 在完整两秒窗口维持物理 `both`，seed 8401/8406/8408 分别在 0.72/0.62/1.70 秒后真实转为 `right_only`；
- 正式 v1 因物理条件实现为 `67/70` 而未通过全部门，硬审计在 seed 8401 正确拒绝；
- 报告明确将失败归因于固定双臂 hold pose 的刚性冲突，不以放松关系阈值修复，也不启动完整双臂策略训练。
