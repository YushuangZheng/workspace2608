...
## 2026-08-05：官方 RoboDojo 边界核验与专家数据复核

按 RoboDojo 官方 Usage 文档重新核对了安装、数据下载、`arx_x5` 配置、`ee` 动作协议和
评测入口。`third_party/RoboDojo` 子模块当前工作树干净，锁定提交为
`25691aa78fb34bbbe798aa3d880d57ef2788f696`；项目只在运行期目录增加适配配置，不修改上游
任务、资产、奖励函数或成功判据。官方 `push_T` 专家 episode 0 已从 Hugging Face 重新下载，
与本地文件逐字节一致（SHA-256
`12c098bb1f5d4d184909f2a9580b5ce36af89b276c4a56bd46e8978051b3ccaf`）。

使用官方 `ee` 动作回放后，机器人末端轨迹与 HDF5 状态保持毫米级一致，但由 RGB 估计的源
