...
Rechecked on 2026-08-21 against upstream commit
...
1. The simulation task, recovery, and compile agents are hard-wired to model
   ...
   generation cannot start. A user-supplied exact GPT-5 API endpoint would meet
   this dependency; substituting a different LLM would change the paper
   protocol and is not accepted.
2. The package requires `dexsim_engine==0.3.11`. It still has no verified
   public HTTPS package or source distribution. The only installation command
   published upstream adds `http://pyp.open3dv.site:2345/simple/` as a trusted
   host. That plaintext private index was deliberately **not contacted** and no
   credential should be sent to it.
3. The documented alternative is a vendor container image, but Docker, Podman,
   Singularity, and Apptainer are absent on this server. A user-space sandbox
   alone does not supply the missing engine or establish its provenance.
...
not by a discovered DynaMAC or local project-code defect. Do not install a
different DexSim release, contact the plaintext index, substitute another
simulator, or substitute another LLM as a workaround. Resume only after the
user provides (a) a securely distributed, provenance-verifiable
`dexsim_engine==0.3.11` package or vendor container plus an approved compatible
runtime and (b) an exact GPT-5 endpoint, credentials, and cost authorization.
Then run `bash scripts/smoke_release.sh --llm` first and accept a single
`SinglePourWater` rollout only if the pinned task loads, the nominal action
completes, the injected failure is observed, recovery is invoked, and logs
contain no dependency fallback. Stop again on any version substitution,
unverifiable binary, credential exposure, or protocol mismatch.
