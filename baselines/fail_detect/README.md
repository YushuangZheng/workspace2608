...
There is no official policy or detector checkpoint and the README omits the
`image_abs.hdf5` data source. The released code also differs from the paper in
conformal alpha, logpZO training epochs, diffusion-policy sample counts, and the
absence of SPARC. Consequently, any run is labelled either `upstream_release`
or `paper_aligned`; neither label is silently substituted for the other.

The original environment is `dynamac-fail-detect` (Python 3.9, PyTorch 1.12.1,
CUDA 11.6). A 24 GB RTX 4090 memory smoke test precedes any long training.
...
on a 4090, occupying about 1.24 GB before batch activations. Training remains blocked at the
data boundary: the release neither contains nor documents the required
`data/robomimic/datasets/transport/ph/image_abs.hdf5`, and no official policy or
detector checkpoint is available. The roughly 85 GB Diffusion Policy image
bundle is a plausible upstream source, but it is not silently treated as an
author-confirmed FAIL-Detect artifact.
