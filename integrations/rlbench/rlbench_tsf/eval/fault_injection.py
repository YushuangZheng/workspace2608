...
The core TSF policy never imports this module. It wraps an RLBench
...
class FaultInjectionSpec:
    ...
    - relation faults: the selected gripper has carried an attached object or
      maintained a detected closed contact for ``minimum_grasped_cycles``
      consecutive policy cycles.
    """
...
class FaultInjectingTaskEnvironment:
    ...
    def _apply_relation_fault(
        ...
        *,
        ...
        gripper.release()
        displaced = False
        # A target-frame displacement is a valid mismatch for a rigidly
        # attached object.  For maintained contact it creates a second scene
        # intervention (and can break support relations to other entities), so
        # the physical fault is only the one-cycle forced release below.
        if (
            self.spec.kind == FaultInjectionKind.RELATION_MISMATCH
    ...
    def _audit_physical_effect(self, *, bimanual: bool) -> None:
        ...
        if self._effect_observed is None:
            ...
            if (
                self.spec.kind == FaultInjectionKind.RELATION_MISMATCH
                and self._relation_trigger_source == "attachment"
            ):
