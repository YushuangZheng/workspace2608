"""Build reproducible TSF sidecars beside, never inside, base-policy models."""
...
DEFAULT_BASE_MODELS = REPOSITORY_ROOT / "integrations/rlbench/models/dynamac_backbone_v1"
DEFAULT_TASK_CONFIG = REPOSITORY_ROOT / "configs/tsf_task_model.json"
DEFAULT_BELIEF_CONFIG = REPOSITORY_ROOT / "configs/tsf_inference.json"
DEFAULT_EXECUTION_CONFIG = REPOSITORY_ROOT / "configs/tsf_execution.json"
DEFAULT_RECOVERY_CONFIG = REPOSITORY_ROOT / "configs/tsf_recovery.json"
DEFAULT_BOUNDARY_ROOT = REPOSITORY_ROOT / "configs/tsf_boundaries"
BUILD_SCHEMA = "essay2608.rlbench.tsf_build.v1"
...
def build_task(
    ...
    *,
    ...
    if output.exists():
        raise FileExistsError(f"拒绝覆盖既有 TSF 模型：{output}")
    paths = tuple(
    ...
    config = TSFPolicyConfig.from_files(
        belief=belief_config,
        execution=execution_config,
        boundary=boundary_config,
        recovery=recovery_config,
    )
    policy = TSFMultiStreamPolicy(models, config)
    ...
    try:
        ...
        atomic_json(
            staging / "build.json",
            {
                ...
                "tsf_output_root": str(output_root.resolve()),
                "files": {
