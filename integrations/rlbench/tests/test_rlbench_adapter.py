...
import numpy as np
import pytest
from essay2608.policy.dynamac import DynaMACConfig, DynaMACDemonstration
...
from integrations.rlbench.rlbench_dynamac.task_specs import (
...
def test_author_aligned_model_config_loads_in_the_current_core() -> None:
    config = DynaMACConfig(**json.loads(AUTHOR_MODEL_CONFIG.read_text(encoding="utf-8")))

    assert config.tau_m == 0.005
    assert config.tau_omega == 0.5
    assert config.eq6_empty_selection == "error"
    assert config.eq6_covariance_scope == "full_pose"
    assert config.policy_model == "time_state"
    assert config.link_mask_scope == "skill_majority"
    assert (config.eq5_position_weight, config.eq5_rotation_weight) == (1.0, 0.0)
