...
def test_training_split_authenticates_exact_nine_cohorts_and_125_files():
    ...
    assert manifest["training_file_count"] == 125
...
def test_training_manifest_cannot_be_resigned_to_read_results(tmp_path):
    manifest = load_training_split_manifest(verify_files=False)
    first = manifest["cohorts"][0]["episodes"][0]["files"]["low_dim_obs"]
    first["path"] = "results/forbidden.json"
    first["sha256"] = "0" * 64
    unsigned = {key: value for key, value in manifest.items() if key != "fingerprint"}
    manifest["fingerprint"] = _canonical_sha256(unsigned)
    path = tmp_path / "training_split_manifest.json"
...
def test_eval_set_paths_are_version_neutral_and_separate_from_training():
    spec = load_evaluation_set_spec()
    assert EVALUATION_SET_ROOT == EVALUATION_SET_ROOT.parents[1] / "data" / "evaluation"
    assert spec["release_scope"] == "v4_complete_materialized_evaluation_inputs"


def test_v4_formal_artifact_paths_keep_inputs_models_and_results_disjoint():
    results = INTEGRATION_ROOT / "results" / "v4" / "diagnostic.json"
    models = INTEGRATION_ROOT / "models" / "v4"
    validate_formal_artifact_paths(output=results, models_dir=models)

    with pytest.raises(ValueError, match="results root"):
        validate_formal_artifact_paths(
            output=EVALUATION_SET_ROOT / "forbidden.json",
            models_dir=models,
        )
    with pytest.raises(ValueError, match="model input"):
        validate_formal_artifact_paths(
            output=results,
            models_dir=INTEGRATION_ROOT / "results" / "v4",
        )
