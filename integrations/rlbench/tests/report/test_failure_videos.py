...
from integrations.rlbench.rlbench_dynamac.report import failure_videos
...
def test_v4_replay_admission_is_cell_scoped_across_global_reseal(monkeypatch):
    manifest = {
        "manifest_sha256": "a" * 64,
        "payload": {
            "spec": {"sha256": "b" * 64},
            "environment_plan_batches": {
                "bimanual_handover_item": {"sha256": "c" * 64}
            },
        },
    }
    batch = {
        "payload": {
            "base_seed": 80,
            "episodes": 1,
            "variation_schedule": [0],
            "batch_fingerprint": "d" * 64,
        },
        "plans": [SimpleNamespace(variation=0)],
    }
    monkeypatch.setattr(
        failure_videos,
        "fixed_environment_plans",
        lambda eval_set_id, task: (manifest, batch),
    )
    source = {
        "release": "v4",
        "task": "bimanual_handover_item",
        ...
        "fixed_eval_set": {
            ...
            # These remain valid provenance even though a different cell
            # caused the current manifest/spec envelope to be resealed.
    ...
    failure_videos._load_sealed_replay_batch(source, "bimanual_handover_item")

    source["fixed_eval_set"]["selected_batch_sha256"] = "9" * 64
    with pytest.raises(RuntimeError, match="cell-selected evaluation batch"):
        failure_videos._load_sealed_replay_batch(
            source, "bimanual_handover_item"
        )
...
def test_default_replay_camera_zoom_changes_only_overhead_fov():
    class Sensor:
        def __init__(self, perspective):
            self.perspective = float(perspective)
            self.pose = [0.0, 0.1, 0.2, 0.0, 0.0, 0.0, 1.0]
            self.set_calls = []
            self.handle_calls = 0
        ...
        def set_perspective_angle(self, value):
            self.set_calls.append(float(value))
            self.perspective = float(value)
    ...
    front = Sensor(60.0)
    overhead = Sensor(80.0)
    environment = SimpleNamespace(
        _scene=SimpleNamespace(
            camera_sensors={"front": front, "overhead": overhead}
        )
    )

    audit = failure_videos._configure_replay_cameras(
        environment,
        ("front", "overhead"),
        failure_videos.DEFAULT_REPLAY_OVERHEAD_PERSPECTIVE_DEGREES,
    )

    assert front.set_calls == []
    assert front.handle_calls == front.capture_calls == 1
    assert front.perspective == 60.0
    assert overhead.set_calls == [70.0]
    assert overhead.handle_calls == overhead.capture_calls == 1
    assert overhead.perspective == 70.0
    assert audit["mode"] == "overhead_zoom_same_pose_front_unchanged_v2"
    assert audit["perspective_degrees_by_camera"] == {
        "front": None,
        "overhead": 70.0,
    }
    assert audit["cameras"]["front"]["perspective_configured"] is False
    assert audit["cameras"]["overhead"]["perspective_configured"] is True
    assert audit["camera_pose_changed"] is False
    assert audit["pre_task_render_primed"] is True
    assert audit["cameras"]["front"]["pre_task_render_primed"] is True
...
def test_replay_attempt_cli_defaults_to_one_and_filename_records_ordinal():
    ...
    assert args.camera_perspective_degrees is None
    assert args.overhead_perspective_degrees == 70.0
    assert (
...
def test_record_replays_same_fixed_episode_fresh_until_first_match(
    ...
    args = SimpleNamespace(
        ...
        camera_perspective_degrees=None,
        overhead_perspective_degrees=None,
        fps=12,
