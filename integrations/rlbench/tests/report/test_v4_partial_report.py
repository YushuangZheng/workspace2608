    ...
    return [
        {
            ...
            "committed_policy_steps": 100,
            "primary_failure_joint_hold_commits": int(episode % 5 == 0),
...
def _store_result(results_root: Path, *, successes: int = 164):
    ...
    return {
        ...
        "controller": {
            **global_ik_controller_metadata(),
            "worker_clock_handshake_id": FORMAL_POLICY_CLOCK_SEMANTICS_ID,
        },
        "scenario_protocol": {
...
def test_open_microwave_task_scoped_protocol_separates_outer_and_runtime_identity(
    scenario,
    dynamic_method,
):
    outer_fingerprint = "a" * 64
    inner_fingerprint = "b" * 64
    payload = {
        "fixed_eval_set": {
            "selected_batch_fingerprint": outer_fingerprint,
        },
        "motion_plan_batch_fingerprint": outer_fingerprint,
        "protocol": {
            "dynamic_method": dynamic_method,
            "protocol_valid": True,
            "motion_protocol": {
                "protocol_id": v4_partial_report.STAGED_VALIDATED_MOTION_PROTOCOL_ID,
            },
            "staged_motion_plan_cache": {
                "schema": v4_partial_report.STAGED_MOTION_PLAN_BATCH_SCHEMA,
                "protocol_id": v4_partial_report.STAGED_VALIDATED_MOTION_PROTOCOL_ID,
                "batch_fingerprint": inner_fingerprint,
    ...
    microwave_cell = v4_partial_report.TARGET_BY_ID[
        f"open_microwave/{scenario}"
    ]

    v4_partial_report._validate_legacy_reused_scenario_protocol(
        payload, microwave_cell
    )

    payload["protocol"]["staged_motion_plan_cache"]["batch_fingerprint"] = (
        outer_fingerprint
    )
    with pytest.raises(
        PartialReportValidationError,
        match="task-scoped OpenMicrowave",
    ):
        v4_partial_report._validate_legacy_reused_scenario_protocol(
            payload, microwave_cell
        )
...
def test_missing_results_stay_not_run_and_never_form_a_full_table(tmp_path):
    ...
    assert report["scope"]["not_run_target_cell_count"] == 22
    assert report["scope"]["full_tables_generated"] is True
    ...
    assert "all 22 locally runnable cells" in markdown
...
def test_replay_sidecars_are_not_discovered_as_formal_results(tmp_path):
    results_root = tmp_path / "results" / "v4"
    ...
    _write_json(
        results_root
        / "replay_video"
        / "bimanual_put_bottle_in_fridge_static_success"
        / "episode_000.json",
        payload,
    )
...
def test_legacy_coordination_filename_is_historical_not_a_current_result(tmp_path):
    results_root = tmp_path / "results" / "v4"
    legacy = (
        results_root
    ...
    discovered = v4_partial_report._discover_result_paths(results_root, {})
...
def test_missing_formal_capture_is_reported_as_post_evaluation_replay_pending(
    tmp_path,
):
    results_root = tmp_path / "results" / "v4"
    payload = _store_result(results_root)
    payload.pop("evaluation_video_capture")
    _write_store_result(results_root, payload)

    report = build_report(results_root=results_root, v3_results_root=tmp_path / "v3")
    store = next(cell for cell in report["cells"] if cell["cell_id"] == STORE_CELL)

    assert store["formal_result_status"] == "COMPLETED_VALIDATED"
    assert store["status"] == "POST_EVALUATION_REPLAY_PENDING"
    assert store["videos"]["status"] == "POST_EVALUATION_REPLAY_PENDING"
    assert report["scope"]["completed_validated_cell_count"] == 1
    assert report["scope"]["post_evaluation_replay_pending_cell_count"] == 1


def test_post_evaluation_replays_report_jobs_and_zero_quota_cells(
    monkeypatch, tmp_path
):
    from integrations.rlbench.rlbench_dynamac.report import (
        v4_post_evaluation_replays,
    )

    results_root = tmp_path / "results" / "v4"
    replay_dir = results_root / "replay_video" / "store_static_success"
    replay_dir.mkdir(parents=True)
    video = replay_dir / "episode_000.mp4"
    video.write_bytes(b"video")
    manifest_path = replay_dir / "manifest.json"
    manifest = {"episodes": [{"video": video.name}]}
    _write_json(manifest_path, manifest)
    ...
    plan = SimpleNamespace(
        ...
        selections=(
            ...
            SimpleNamespace(
                cell_id="stack_wine/static",
                quota_tier="paper_close_zero",
    ...
    validated = []
    monkeypatch.setattr(v4_partial_report, "DEFAULT_RESULTS_ROOT", results_root)
    monkeypatch.setattr(
        v4_post_evaluation_replays,
    ...
    monkeypatch.setattr(
        v4_post_evaluation_replays, "_validate_job_output", validate
    )

    summaries = v4_partial_report._validated_post_evaluation_replays(
        results_root=results_root,
        completed_cell_count=22,
    )

    assert validated == [store_job]
    assert summaries[STORE_CELL]["status"] == "REPLAY_VALIDATED"
    assert summaries[STORE_CELL]["retained"] == {
        "successes": 1,
        "failures": 0,
    }
    success = summaries[STORE_CELL]["outcome_manifests"]["success"]
    assert success["manifest_path"].endswith("/manifest.json")
    assert success["manifest_sha256"] == _sha256(manifest_path)
    assert success["confirmed_trajectories"] == 1
    assert summaries["stack_wine/static"]["status"] == "REPLAY_VALIDATED"
    assert summaries["stack_wine/static"]["retained"] == {
        "successes": 0,
        "failures": 0,
    }
    assert summaries["stack_wine/static"]["outcome_manifests"] == {}
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ("evaluation_protocol_id", "legacy-v3", "formal V4"),
    ),
)
...
@pytest.mark.parametrize("field", ("manifest_sha256", "spec_sha256"))
def test_global_seal_hashes_are_provenance_not_cross_cell_admission_keys(
    tmp_path, field
):
    results_root = tmp_path / "results" / "v4"
    payload = _store_result(results_root)
    ...
    report = build_report(
        results_root=results_root, v3_results_root=tmp_path / "v3"
    )
    store = next(cell for cell in report["cells"] if cell["cell_id"] == STORE_CELL)
    assert store["formal_result_status"] == "COMPLETED_VALIDATED"
...
@pytest.mark.parametrize("field", ("manifest_sha256", "spec_sha256"))
def test_global_seal_hash_provenance_must_still_be_a_sha256(tmp_path, field):
    results_root = tmp_path / "results" / "v4"
    payload = _store_result(results_root)
    payload["fixed_eval_set"][field] = "not-a-sha"
    _write_store_result(results_root, payload)

    with pytest.raises(PartialReportValidationError, match=rf"{field}.*SHA-256"):
        build_report(results_root=results_root, v3_results_root=tmp_path / "v3")


@pytest.mark.parametrize(
    "field", ("selected_batch_sha256", "selected_batch_fingerprint")
)
def test_only_a_changed_cell_selected_batch_makes_that_result_pending(
    tmp_path, field
):
    results_root = tmp_path / "results" / "v4"
    payload = _store_result(results_root)
    payload["fixed_eval_set"][field] = "9" * 64
    path = _write_store_result(results_root, payload)
    ...
    assert store["formal_result_status"] == "PENDING"
    assert store["result_path"] == str(path.relative_to(results_root))
    assert store["success_rate"] is None
    assert report["scope"]["completed_validated_cell_count"] == 0
    assert "cell-selected evaluation batch" in store["pending_reason"]
    assert "Status: **PENDING**" in render_markdown(report)
...
def test_fixed_bottle_only_zero_count_subgroups_allow_null_rate(tmp_path):
    results_root = tmp_path / "results" / "v4"
    payload = _store_result(results_root)
    payload["store_mode_subgroups"] = {
        "bottle_only": {
    ...
    report = build_report(results_root=results_root, v3_results_root=tmp_path / "v3")
    store = next(cell for cell in report["cells"] if cell["cell_id"] == STORE_CELL)
    assert store["store_subgroups"]["groups"]["fridge_only"]["success_rate"] is None
    assert store["store_subgroups"]["groups"]["both"]["success_rate"] is None
...
def test_writer_refuses_to_put_result_state_in_evaluation_set(monkeypatch, tmp_path):
    ...
    forbidden = tmp_path / "data" / "evaluation" / "report.json"
