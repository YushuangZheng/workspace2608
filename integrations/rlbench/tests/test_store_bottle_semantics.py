...
def test_v4_store_semantics_reuse_the_pinned_ttm_without_touching_v1() -> None:
    ...
    assert spec.training_data_root == "integrations/rlbench/data/v4/store_bottle"
...
def test_semantic_groups_match_the_audited_legacy_ttm_hierarchy() -> None:
    batch = json.loads(LEGACY_PLAN_BATCH.read_text(encoding="utf-8"))
    rows = batch["plans"][0]["validation"]["source_task_tree_relative_state"]
...
def test_hierarchy_audit_rejects_a_waypoint_attached_to_the_wrong_group() -> None:
    ...
    rows = [
        dict(row)
        for row in batch["plans"][0]["validation"][
