...
def test_frozen_v4_diagnostic_schema_remains_unchanged() -> None:
    assert initialize_ik_solver_diagnostics() == {
        "jacobian_failures": 0,
...
def test_formal_protocol_is_distinct_while_legacy_v3_id_remains_available(
    module,
) -> None:
    ...
    assert (
        "pseudo6-then-bounded-trac-distance-then-collision-aware-sampling5-"
        "nearest-current-q-then-far10cm-collision-aware-path"
    ) in current
    assert "single-primary-request-raw-joint-hold-same-transaction" in current
    assert "contact-delta-diagnostic-v5" in current
    assert "contact-delta-diagnostic-v3" in legacy
    assert "collision-aware" not in legacy
    assert (
        module.LEGACY_V3_EVALUATION_PROTOCOL_ID
        == module.legacy_v3_evaluation_protocol_id(3)
    )
