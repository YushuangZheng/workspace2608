...
from integrations.rlbench.rlbench_dynamac import runtime
...
from integrations.rlbench.rlbench_dynamac import v4_dynamic_protocol as protocol
...
def test_v4_protocols_freeze_requested_ticks_motion_and_identity():
    ...
    assert coord["motion"]["cartesian_substeps"] == 10
    assert coord["motion"]["persistent_policy_target_offset"] is False
    assert coord["clock_semantics"]["policy_clock_advances_during_substeps"] is False
...
def test_v4_coordination_command_holds_other_arm_grippers_and_orientation():
    observation = _observation(left_open=1.0, right_open=0.0)
    measured = observation.left.gripper_pose.copy()

    command = coordination._v4_coordination_cartesian_command(
        observation,
        arm="left",
        measured_pose=measured,
        fraction=0.4,
    )

    np.testing.assert_allclose(command[:7], observation.right.gripper_pose)
    assert command[7:9].tolist() == [0.0, 0.0]
    np.testing.assert_allclose(command[9:12], measured[:3] + [0.0, 0.0, 0.012])
    np.testing.assert_allclose(command[12:16], measured[3:7])
    assert command[16:18].tolist() == [1.0, 0.0]


def test_v4_coordination_executes_ten_substeps_and_refreshes_without_policy_clock():
    ...
    final, reward, terminate, audit = coordination._apply_v4_coordination_intervention(
        task,
        initial,
        arm="right",
        max_attempts=3,
        invalid_action_error=InvalidAction,
    )

    assert reward == 0.0 and terminate is False
    assert len(task.commands) == 10
    assert task.refreshes == 1
    assert audit["completed"] is True
    assert audit["cartesian_substeps_completed"] == 10
    assert audit["policy_requests"] == 0
    assert audit["policy_clock_advances"] == 0
    assert audit["persistent_offset"] is False
    np.testing.assert_allclose(
        final.right.gripper_pose[:3],
        initial.right.gripper_pose[:3] + [0.0, 0.0, 0.03],
    )
    np.testing.assert_allclose(
        final.right.gripper_pose[3:7],
        initial.right.gripper_pose[3:7],
    )


def test_v4_coordination_retries_same_substep_three_times_then_fails_closed():
    class InvalidAction(Exception):
        pass
    ...
    initial = _observation()
    task = TaskEnvironment(initial)
    _observation_after, reward, terminate, audit = (
        coordination._apply_v4_coordination_intervention(
            task,
            initial,
            arm="left",
            max_attempts=3,
    ...
    assert audit["retry_exhausted"] is True
    assert audit["cartesian_substeps_completed"] == 0
    assert audit["action_attempts"] == 3
    assert audit["policy_clock_advances"] == 0
...
def test_v4_coordination_run_applies_once_before_that_tick_policy_request(
    monkeypatch,
):
    ...
    initial = _observation()
    policy_action = coordination._noop_action(initial)
    ...
    class Worker:
        def request(self, command, *args, **kwargs):
            ...
            if command == "reset":
                return {"ok": True}
            if command == "act":
                # All ten physical substeps must precede this tick's prediction.
                assert len(task.commands) == 10
                return {"action": policy_action.copy(), "transaction_id": 1}
            if command == "commit":
    ...
    class TaskEnvironment:
        ...
        def step(self, command):
            ...
            reward = 1.0 if len(self.commands) == 11 else 0.0
    ...
    assert calls == ["reset", "act", "commit"]
    assert len(task.commands) == 11
    # The first policy action after the one-shot push is not offset again.
    np.testing.assert_allclose(task.commands[-1], policy_action)
    assert result["success"] is True
    assert result["committed_policy_steps"] == 1
    assert result["coordination_intervention"]["cartesian_substeps_completed"] == 10
    assert result["coordination_intervention"]["persistent_offset"] is False
