...
V4_COORDINATION_PROTOCOL_ID = (
    "handover-measured-ee-world-z-cartesian-push-once-v4"
)

V4_LIFT_TRIGGER_STEP = 35
V4_COORDINATION_TRIGGER_STEP = 235
V4_COORDINATION_CARTESIAN_SUBSTEPS = 10
...
def load_v4_coordination_intervention_protocol(
    ...
    if (
        ...
        or motion.get("measurement_source")
        != "current_observation_gripper_pose"
        or motion.get("translation_frame") != "world"
        or motion.get("translation_m")
        != list(V4_COORDINATION_TRANSLATION_METERS)
        or motion.get("cartesian_substeps")
        != V4_COORDINATION_CARTESIAN_SUBSTEPS
        or motion.get("orientation")
        != "fixed_at_measured_pre_intervention_orientation"
        or motion.get("other_arm")
        != "hold_current_measured_pose_each_substep"
        or motion.get("both_grippers")
        != "hold_current_discrete_state_each_substep"
        or motion.get("application")
        != "one_shot_then_resume_unmodified_policy"
        or motion.get("persistent_policy_target_offset") is not False
        or clock
        != {
            "policy_requests_during_substeps": 0,
            "policy_clock_advances_during_substeps": False,
            "observation_refreshed_after_substeps": True,
        }
        or not isinstance(invalid, dict)
        or invalid.get("max_attempts_per_cartesian_substep") != 3
        or invalid.get("solver")
        != "v4_collision_aware_jacobian_then_sampling"
        or invalid.get("exhaustion_reason")
        != "coordination_intervention_retry_exhausted"
        or not isinstance(provenance, dict)
        or provenance.get("result_based_retuning_forbidden") is not True
        or provenance.get("legacy_v3_profile_unchanged") is not True
    ):
...
def attach_v4_lift_plan_evidence(plan: Any) -> Any:
    ...
    from .runtime import StagedMotionPlan
...
def stage_v4_lift_motion_plan(
    ...
    *,
    ...
    from .runtime import stage_scenario_motion_plan
...
def load_v4_lift_motion_plan_batch(payload: dict[str, Any]) -> list[Any]:
    ...
    from .runtime import load_staged_motion_plan_batch
...
def build_v4_lift_task_scoped_plan_batch(
    *,
    ...
    from .runtime import staged_motion_plan_batch
...
def v4_coordination_trigger_authentication(
    *,
    ...
    if (
        isinstance(policy_steps, bool)
        or not isinstance(policy_steps, int)
        or policy_steps <= V4_COORDINATION_TRIGGER_STEP
    ):
        raise ValueError("V4 coordination trigger lies outside the policy clock")
    return {
