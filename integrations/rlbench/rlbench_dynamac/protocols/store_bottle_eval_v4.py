...
_V4_STORE_SEMANTIC_IDENTITY_FINGERPRINT = (
    "1cd04a4c39794884677fa43e7a88bc86e9b9e7f266ce573d8642d85631cc09cc"
)
_V4_STORE_IDENTITY_COLLECTION_MANIFEST_PATH = (
    "integrations/rlbench/data/v4/store_bottle/collection_manifest.json"
)
_V4_STORE_CURRENT_COLLECTION_MANIFEST_PATH = (
...
V4_STORE_FORMAL_MODE = "bottle_only"
V4_STORE_FORMAL_MODE_SCHEDULE = "fixed_bottle_only"
V4_STORE_FORMAL_N200_MODE_COUNTS = {
    "bottle_only": 200,
    "fridge_only": 0,
    "both": 0,
}
V4_STORE_GOAL_SAMPLING_MAX_ATTEMPTS = 1000
...
def _load_config(path: Path, schema: str) -> dict[str, Any]:
    ...
    if schema == V4_STORE_INTERVENTION_SCHEMA:
        current_path = payload.get("evidence", {}).get("collection_manifest_path")
        if current_path != _V4_STORE_CURRENT_COLLECTION_MANIFEST_PATH:
            raise ValueError("StoreBottle V4 collection-manifest path is invalid")
        identity_payload = json.loads(json.dumps(payload))
        identity_payload["evidence"][
            "collection_manifest_path"
        ] = _V4_STORE_IDENTITY_COLLECTION_MANIFEST_PATH
    return {**payload, "fingerprint": canonical_fingerprint(identity_payload)}
...
def load_v4_store_motion_source_protocol(
    ...
    *,
    ...
    if (
        ...
        or payload.get("goal_sampling_max_attempts")
        != V4_STORE_GOAL_SAMPLING_MAX_ATTEMPTS
        ...
        or schedule
        != {
            "formula": V4_STORE_FORMAL_MODE_SCHEDULE,
            "mode": V4_STORE_FORMAL_MODE,
            "formal_n200_counts": V4_STORE_FORMAL_N200_MODE_COUNTS,
        }
    ...
    if verify_semantics_file:
        ...
        semantic_spec = load_store_bottle_semantic_spec(semantic_path)
        raw_hash_matches = _file_sha256(semantic_path) == semantics.get(
...
class StoreBottleMultiEntityPlan:
    ...
    def __post_init__(self) -> None:
        ...
        if self.mode not in V4_STORE_MODE_ORDER:
            raise ValueError("StoreBottle plan mode is unsupported")
...
def store_bottle_motion_plan_batch(
    *,
    ...
    for episode, (variation, plan) in enumerate(zip(variations, plans)):
        ...
        counts[plan.mode] += 1
    if len(plans) == 200 and counts != V4_STORE_FORMAL_N200_MODE_COUNTS:
    ...
    body = {
        ...
        "mode_schedule": V4_STORE_FORMAL_MODE_SCHEDULE,
        "mode_counts": counts,
...
def load_v4_store_motion_plan_batch(payload: dict[str, Any]) -> list[Any]:
    ...
    if (
        ...
        or payload.get("mode_schedule") != V4_STORE_FORMAL_MODE_SCHEDULE
...
def v4_store_task_class() -> Any:
    ...
    from integrations.rlbench.rlbench_dynamac.store_bottle_live_v4 import (
        BimanualPutBottleInFridgeSemanticV4,
    )
...
def stage_v4_store_motion_plan(
    ...
    *,
    ...
    episode_seed: int,
    variation: int,
    max_attempts: int = V4_STORE_GOAL_SAMPLING_MAX_ATTEMPTS,
