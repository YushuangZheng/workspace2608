...
EVALUATION_SET_ROOT = INTEGRATION_ROOT / "data" / "evaluation"
TRAINING_SPLIT_MANIFEST = INTEGRATION_ROOT / "data" / "training" / "manifest.json"
EVALUATION_SET_SPEC = EVALUATION_SET_ROOT / "spec.json"
EVALUATION_SET_V2_ID = "rlbench_eval_v2"
EVALUATION_SET_V2_CONFIG_SPEC = EVALUATION_SET_SPEC
...
_REGENERATED_V2_TASK_CONSUMERS = {
    "open_microwave": ["static", "smooth", "teleport"],
    "bimanual_put_bottle_in_fridge": ["static", "teleport"],
    "bimanual_lift_tray": ["static", "teleport"],
}
...
def validate_training_entry_paths(*paths):
    ...
    forbidden = (EVALUATION_SET_ROOT.resolve(), (INTEGRATION_ROOT / "results").resolve())
    for value in paths:
...
def load_training_split_manifest(path=TRAINING_SPLIT_MANIFEST, verify_files=True):
    ...
    if (
        ...
        or
        payload.get("split_id") != "rlbench_v4_current_training_inputs"
    ...
    authenticated_inputs = {
        "source_collection_manifests": {
            "data/training/main/collection_manifest.json",
            "data/training/main/bimanual_put_bottle_in_fridge/collection_manifest.json",
            "data/training/main/bimanual_sweep_to_dustpan/augmentation_manifest.json",
            "data/training/coordination/collection_manifest.json",
        },
        "configuration_identity": {
    ...
    expected = {
        ...
        ("store_bottle_v4", "bimanual_put_bottle_in_fridge"),
        ("sweep_dust_v4", "bimanual_sweep_to_dustpan"),
        ("table_iii_coordination", "bimanual_handover_item_dynamic"),
    }
    if set(seen_tasks) != expected or total != 45 or len(seen_paths) != 125:
        raise ValueError("training cohort identity set is invalid")
    return payload


def load_evaluation_set_spec(path=EVALUATION_SET_SPEC):
    """Compatibility name for the sole current V2 evaluation spec."""

    return load_evaluation_set_v2_spec(path)
...
def load_evaluation_set_v2_spec(path=EVALUATION_SET_V2_CONFIG_SPEC):
    ...
    if (
        ...
        != "v4_complete_materialized_evaluation_inputs"
    ...
    if payload.get("training_identity_contract") != {
        "kind": "direct_training_manifest_binding",
        "schema": "dynamac-rlbench-training-data-binding-v1",
        "path": "data/training/manifest.json",
        "training_episode_count": 45,
        "training_file_count": 125,
        "training_model_group_count": 9,
    }:
    ...
    if (
        ...
        != "historical_provenance_with_local_materialization"
        or legacy.get("source_is_read_only") is not True
        or legacy.get("source_artifacts_remain_external") is not False
    ...
    if protocols != {
        "reused_legacy": {
            ...
            "identity_mode": "legacy_global_identity_preserved_in_local_bytes",
        },
        "regenerated_v2": {
    ...
    for task, profile in dynamic.items():
        ...
        if (
            ...
            != (
                _REGENERATED_V2_TASK_CONSUMERS[task]
                if regenerated
        ...
        if (
            ...
            or not artifact_path.startswith("environment/")
        ):
        ...
        if regenerated:
            ...
            legacy_path = profile.get("legacy_source_artifact_path")
            if (
                not isinstance(legacy_path, str)
                or not legacy_path.startswith("plans/environment/")
                or Path(legacy_path).name != Path(artifact_path).name
            ):
                raise ValueError(
    ...
    if (
        ...
        != (
            ...
            "bimanual_handover_item_dynamic_a_only_n200.json"
        )
        or not isinstance(coordination_profile.get("artifact_path"), str)
        or not coordination_profile["artifact_path"].startswith("coordination/")
    ):
    ...
    if payload.get("isolation") != {
        ...
        "evaluation_artifact_root": "data/evaluation",
